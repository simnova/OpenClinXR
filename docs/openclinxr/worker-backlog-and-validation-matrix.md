...
- Do not create a no-install `apps/ui-xr-iwsdk-spike` scaffold. The sidecar is currently contract-only and should become a runnable app only after operator install-scope approval, exact IWSDK versions, accepted transitive license posture, and a passing `pnpm iwsdk:verify` lane.
- Before any install-backed sidecar change, run the proposed dependencies through the planning package's pre-install policy. The first slice allows only exact-versioned `@iwsdk/core` and `@iwsdk/xr-input`; `@iwsdk/locomotor`, `@iwsdk/vite-plugin-dev`, and `@iwsdk/vite-plugin-gltf-optimizer` require extra review and are not unattended-ready; `@iwsdk/reference`, `@meta-quest/hzdb`, blocked copyleft/unknown license expressions, and any sharp/libvips transitive variant remain blockers.
- IWSDK agent-tooling readiness now requires more than a smoke sequence: record `iwsdk adapter sync`, the 32-tool MCP inventory, category coverage for session/transforms/input/browser/scene/ECS, scored browser-mode evidence from `pnpm iwsdk:browser:evidence`, and optional MCP server policy for `iwsdk-reference` and `hzdb`.
