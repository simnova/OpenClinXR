...
- IWSDK agent-tooling readiness now requires more than a smoke sequence: record `iwsdk adapter sync`, the 32-tool MCP inventory, category coverage for session/transforms/input/browser/scene/ECS, and optional MCP server policy for `iwsdk-reference` and `hzdb`.
