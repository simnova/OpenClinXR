...
- Before any install-backed sidecar change, run the proposed dependencies through the planning package's pre-install policy. The first slice allows only exact-versioned `@iwsdk/core` and `@iwsdk/xr-input`; `@iwsdk/locomotor`, `@iwsdk/vite-plugin-dev`, and `@iwsdk/vite-plugin-gltf-optimizer` require extra review; `@iwsdk/reference`, `@meta-quest/hzdb`, blocked copyleft/unknown license expressions, and the sharp/libvips transitive path remain blockers.
