...
2026-05-27 Worker tooling/dependency hardening slice: IWSDK npm currentness now reflects latest package metadata (`@iwsdk/*` `0.4.1`, `@meta-quest/hzdb` `1.2.1`) and still blocks runtime adoption because Vite plugins peer on `vite: ^7.0.0` while the repo uses Vite `8.0.10`. Asset-production readiness now accepts either `gltf-pipeline` smoke evidence or `@gltf-transform/core` Node API smoke evidence, and local-runtime probing can satisfy the GLTF conversion runtime gate from the Node API package. Evidence: `docs/openclinxr/iwsdk-npm-currentness-2026-05-27.json`, `docs/openclinxr/gltf-transform-smoke-2026-05-27.json`, `docs/openclinxr/asset-production-readiness-benchmark-2026-05-27.json`. Next queued slice: security/license doc alignment and e18e-backed `gltf-pipeline` removal/isolation decision.
