...
## Queue Stages, Retry Checkpoints, And Shared Library Reuse

The queue tool now emits deterministic operational notes alongside each worker execution. Those notes are intended for offline review and do not imply any live provider, cloud worker, or production readiness claim.

- Stage flow is still `queued -> running -> succeeded | failed | review_blocked`.
- `queued_to_running` marks the handoff into the durable stage checkpoint.
- `running_to_succeeded` means the stage can advance after its checkpoint is written.
- `running_to_failed` keeps the stage retryable from the durable checkpoint.
- `running_to_review_blocked` pauses the stage at a human review gate instead of auto-retrying.
- Shared library hits use `reuse_cached_asset` and skip regeneration.
- Shared library misses use `regenerate_shared_asset`, and LRU-eviction misses use `regenerate_after_lru_eviction`.
- The execution payload surfaces these notes in `operationalNotes.stageNotes` and `operationalNotes.sharedLibraryNotes` so queue operators can see the retry boundary without consulting live provider state.
...
## 2026-05-21 ED bay environment artifact checkpoint

The ED chest-pain station now has a local repo-authored environment evidence bundle generated by `pnpm asset:environment:generate` and validated by `pnpm asset:environment:validate`.
...
- `docs/openclinxr/environment-artifacts-YYYY-MM-DD.json`

The WebXR app serves the shell at `/xr-assets/environment/ed-exam-bay-shell.glb` and loads it as `openclinxr.ed-chest-pain.environment-shell.generated-glb`, included in copied `sceneAssetEvidence`. This is local fixture evidence for visual iteration only; it is not production environment readiness, Quest readiness, clinical visual validity, or scoring evidence.
