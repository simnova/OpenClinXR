...
- `packages/openclinxr/graphql` now avoids runtime `.graphql` file reads in bundled API code by exporting static TypeScript schema and operation constants. Tests compare the embedded strings against the source `.graphql` files until a later GraphQL Code Generator/Cellix `static-type-defs`-style artifact replaces the hand-maintained constants.
