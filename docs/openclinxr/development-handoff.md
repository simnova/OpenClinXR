...
- Treat the Ant Design CLI skill as optional local support for AntD 6 component work. Prefer package-managed execution, use `--format json`, verify exact-version component APIs/tokens/demos before authoring UI, and run lint/doctor checks after substantial admin component edits.
...
Current tooling note: `turbo`, `knip`, and `@e18e/cli` are installed as pnpm-managed root dev dependencies. `pnpm packages:typecheck` and `pnpm packages:test` now delegate package work to `turbo run` with telemetry opt-out variables set in the scripts, while root-only agent/source/audit checks stay outside Turbo. `packages/cellix/config-typescript` has no package-local test/typecheck script and is safely skipped by Turbo, matching the previous `pnpm -r --if-present` behavior. `pnpm hygiene:knip` is report-only for now. `pnpm hygiene:e18e:help` verifies the E18E CLI entrypoint. `pnpm hygiene:e18e:analyze` is intentionally outside the main `verify` gate because a whole-repo spike on 2026-05-03 did not return promptly and needs a scoped baseline before becoming blocking.
...
- `apps/ui-admin` has typed control-plane client methods and an exam-forms workbench action for listing and creating seed station-run queue review snapshots through shared generated admin GraphQL documents.
...
- `packages/openclinxr/graphql` includes `createStationRunQueueSnapshot` and `stationRunQueueSnapshots` operation contracts, exports the generated documents through `@openclinxr/graphql/documents`, and keeps GraphQL Code Generator/Apollo adoption aligned with the same review-snapshot shape.
...
- `turbo.json` now orchestrates package-level `typecheck`, `test`, `build`, and `dev` tasks. Root `typecheck` and `test` delegate package work through `turbo run`, while root agent-factory, source-ledger, audit, and license checks remain explicit root commands. Architecture rules now enforce Turbo delegation, telemetry opt-out variables, and no tracked `.turbo` cache artifacts.
