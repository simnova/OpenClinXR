...
- `pnpm local:runtime:probe` produced `docs/openclinxr/local-runtime-probe-2026-05-04.json`: Quest USB is ready, but the latest headset wakefulness sample is asleep; Homebrew, Node 22, pnpm 10, Python 3.11, ffmpeg, adb, Blender, `llama.cpp`, `vibevoice`, and the pinned Apache-2.0 `gltf-pipeline` CLI are visible on this machine.
...
- Treat Meta Immersive Web SDK as an XR spike candidate, not a default runtime dependency. `apps/ui-xr-iwsdk-spike` now exists as the approved Phase 1 sidecar with `@iwsdk/core@0.3.1`, `@iwsdk/xr-input@0.3.1`, `three@0.184.0`, and a root pnpm override pinning Three. It renders the ED chest pain shell, exposes IWSDK export counts, preserves the scene-name parity contract, and passed a Chrome DevTools smoke with nonblank WebGL canvas, `ecg_request` trace advancement, and no warning/error console messages after adding the favicon. It is not production-ready: `docs/openclinxr/iwsdk-sidecar-phase1-metrics-2026-05-04.json` records `app_js_bundle_kb_over_budget`, `bundle_delta_vs_ui_xr_kb_over_budget`, and missing foreground Quest frame/latency metrics. The reviewed Vite plugins still peer Vite `^7.0.0` while OpenClinXR uses Vite `8.0.10`; `@iwsdk/reference` has docs-vs-npm metadata drift (`v0.3.1` docs, `0.3.2` npm latest); and optional `@meta-quest/hzdb` reports `UNLICENSED` in npm metadata. Patrick approved reference warmup scope and `@meta-quest/hzdb` legal/procurement posture on 2026-05-04, but those approvals do not authorize production manifests or default verification. Keep `@iwsdk/create`, `@iwsdk/reference`, `@iwsdk/starter-assets`, `@meta-quest/hzdb`, and Phase 2 IWSDK Vite plugins out of the sidecar until their gates are deliberately revised. The sidecar must not import or depend on `@openclinxr/ui-xr` or `apps/ui-xr/src/**`. `docs/openclinxr/iwsdk-codex-mcp-runbook.md` is the human runbook for the future local adapter. `pnpm iwsdk:verify` is the opt-in IWSDK policy lane and now runs the workspace posture check with `--approved-sidecar`.
