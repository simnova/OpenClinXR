...
- `pnpm xr:quest:manual:check`: validates a foreground in-headset manual performance report, or records `missing_quest_manual_performance_report` until one exists. Use `docs/openclinxr/quest-manual-performance-template.json` as the capture template; the checker now requires operator identity, strict ISO timestamp, foreground page, screencast disabled, extra windows closed, immersive session confirmation, at least 600 observed frames, a rolling frame window of at least 120 samples from `window.__openClinXrFrameStats`, battery-drop entry, comfort, and the FPS/frame-time thresholds before frame-pacing claims can clear.
