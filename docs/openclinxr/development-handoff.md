...
- `llama.cpp` 9010 was installed locally on 2026-05-04 through Homebrew for model-runtime evidence only. `llama-cli` and `llama-server` are available with Metal backend initialization on this Apple Silicon machine, and `docs/openclinxr/local-model-runtime-benchmark-2026-05-04.json` records the first approved local `Qwen/Qwen3-4B-GGUF` benchmark. `pnpm local:model:runtime -- --parse-log <local-log> --output <report.json>` now harvests an existing local llama.cpp log into the same report shape without executing inference, downloading weights, using cloud APIs, or making production/clinical/target-hardware claims; because it is parse-only, the report `generatedAt` remains the original runtime timestamp and stale-evidence gates still require a genuinely fresh local runtime run when leadership proof is needed.
...
- `pnpm local:runtime:probe` produced `docs/openclinxr/local-runtime-probe-2026-05-05.json`: Quest USB is ready, Bun 1.3.13 is available at `/Users/patrick/.bun/bin/bun`, and Homebrew, Node 22, pnpm 10, Python 3.11, ffmpeg, adb, Blender, `llama.cpp`, `vibevoice`, `portless@0.12.0` at `/Users/patrick/Library/pnpm/portless`, and the pinned Apache-2.0 `gltf-pipeline` CLI are visible on this machine. The local Bun WebSocket smoke in `docs/openclinxr/api-bun-websocket-runtime-smoke-2026-05-05.json` passed for `/voice/realtime/ws` with HTTP/3/WebTransport/QUIC/Web3 still out of scope.
