...
- `pnpm local:runtime:probe` produced `docs/openclinxr/local-runtime-probe-2026-05-05.json`: Quest USB is ready, the latest headset wakefulness sample was asleep so foreground preflight remains blocked, Bun is not installed, and Homebrew, Node 22, pnpm 10, Python 3.11, ffmpeg, adb, Blender, `llama.cpp`, `vibevoice`, `portless@0.12.0` at `/Users/patrick/Library/pnpm/portless`, and the pinned Apache-2.0 `gltf-pipeline` CLI are visible on this machine.
