...
- `packages/openclinxr/graphql` now avoids runtime `.graphql` file reads in bundled API code by generating static TypeScript schema and operation artifacts. Package tests run `generate:check` before Vitest so source `.graphql` files cannot drift from the bundled artifacts.
- `apps/api` now injects a no-op or in-memory telemetry recorder through the CellixJS-inspired startup path and records sanitized route-level spans with stable REST route IDs, status codes, duration, and station-run IDs where applicable. Request bodies, learner utterances, prompts, patient notes, and hidden facts are intentionally excluded from telemetry attributes.
