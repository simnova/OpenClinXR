...
- `pnpm local:runtime:probe` produced `docs/openclinxr/local-runtime-probe-2026-05-04.json`: Quest USB, Homebrew, Node 22, pnpm 10, Python 3.11, ffmpeg, adb, Blender, and the pinned Apache-2.0 `gltf-pipeline` CLI are ready on this machine. Bun, Portless, local model runtime, and local voice runtime are still marked missing or `not_configured` until installed and benchmarked.
