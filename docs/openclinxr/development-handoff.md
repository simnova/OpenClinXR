...
- `docs/openclinxr/local-model-runtime-benchmark-2026-05-06.json` records the latest repo-managed, cache-only local `Qwen/Qwen3-4B-GGUF` run through `llama.cpp` on Apple M1 Max Metal: about 24.82 generated tokens/sec, about 445 ms approximate time to first generated token, and parsable JSON with schema/extraction caveats. `docs/openclinxr/local-model-quality-benchmark-2026-05-06.json` now points at that fresh runtime report and the approved local Qwen actor-policy probe report; the local model leadership gate remains blocked on target M4 Pro/Max hardware evidence, not stale runtime evidence. This is still only a local adapter latency/structured-output/actor-policy smoke.
...
- `pnpm agent:verify`: agent-factory gate covering memory index, source ledger, pinned dependencies, stale loop-plan detection, risk/evidence debt, maturity, benchmark evidence reporting, GLTF conversion smoke validation, Blender bake evidence validation, asset capability evidence validation, asset production readiness validation, asset ladder validation, and artifact-backed production asset evidence validation.
...
- `pnpm asset:gltf:smoke:validate`: validates the committed GLTF conversion smoke schema, GLB header fields, and verdict blocker consistency without running `gltf-pipeline`.
...
- `pnpm asset:blender:bake:validate`: validates the committed Blender bake smoke schema, GLB header fields, semantic inventory, and verdict blocker consistency without launching Blender.
- `pnpm asset:capability:evidence:validate`: validates the committed deterministic asset capability job evidence shape, capability coverage, policy booleans, per-job provenance fields, and verdict blocker consistency without starting new jobs.
- `pnpm asset:production:readiness:validate`: validates the committed production readiness benchmark schema, proof lane coverage, policy booleans, claim boundaries, runtime budget fields, and verdict blocker consistency without generating assets.
...
- `pnpm asset:production:artifact-evidence:validate`: validates the committed artifact-backed production asset evidence manifest, canonical lane coverage, materialized file claims, policy booleans, and blocked verdict consistency without generating or downloading assets.
...
- `pnpm local:model:cache`: records local reasoning-model weight cache evidence without downloading or executing inference. The 2026-05-06 report proves the approved `Qwen/Qwen3-4B-GGUF` `Qwen3-4B-Q4_K_M.gguf` file exists under the Hugging Face cache with revision `bc640142c66e1fdd12af0bd68f40445458f3869b`, byte size, SHA-256, and Apache-2.0 source posture. `pnpm local:model:cache:validate` is in `pnpm agent:verify`, and `pnpm agent:benchmarks` now surfaces this report as `local_model_cache_evidence` for local model quality gates.
- `pnpm local:model:source-currentness:validate`: validates the latest committed Hugging Face metadata snapshot against the latest local model cache evidence without network access. `docs/openclinxr/local-model-source-currentness-2026-05-06.json` records that the approved Qwen GGUF cache still matches the captured source SHA, Apache-2.0 license posture, and expected `Qwen3-4B-Q4_K_M.gguf` listing.
- `pnpm local:voice:model-cache`: records local realtime voice model cache evidence for the approved developer-only Moshi/Qwen voice candidates. The 2026-05-06 report now records Hugging Face metadata revisions embedded in the local cache directories, so cached weights can be compared to captured source metadata without rerunning downloads or inference.
- `pnpm local:voice:model-source-currentness:validate`: validates `docs/openclinxr/local-realtime-voice-model-source-currentness-2026-05-06.json` against the latest committed Hugging Face metadata snapshot and local voice cache evidence. It confirms the cached `kyutai/moshiko-mlx-q4` and `mlx-community/Qwen3-TTS-12Hz-0.6B-Base-4bit` revisions match the captured source SHAs and accepted license posture, while still making no live-dialog or production-readiness claim. `pnpm agent:benchmarks` now surfaces this as `local_realtime_voice_model_source_currentness` and keeps ready cache evidence blocked from currentness claims if the source-currentness report is missing, stale, or failed.
