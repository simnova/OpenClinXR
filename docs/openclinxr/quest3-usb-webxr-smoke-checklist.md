...
- Thumbstick, keyboard, room-scale, or deliberate hand-gesture locomotion changes the rig position or turn angle without leaving the bounded bay; hand-only runs should record `activeLocomotionSource: "xr_hand_gesture"` and preserve `locomotionDelta` when that path is used. If no movement occurs, keep the copied `input.locomotionDiagnostics` payload so engineering can see whether axes were missing/below deadzone, pinch was not detected, joints were missing, dwell was still arming, or gesture movement stayed below deadzone.
