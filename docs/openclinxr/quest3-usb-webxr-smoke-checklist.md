...
- Thumbstick, keyboard, room-scale, or deliberate hand-gesture locomotion changes the rig position or turn angle without leaving the bounded bay; hand-only runs should record `activeLocomotionSource: "xr_hand_gesture"` and preserve `locomotionDelta` when that path is used. For hand tracking, pinch thumb to index and hold the whole hand steady past the 450 ms dwell interval, then move the still-pinched whole hand through space. Move either hand forward/back for forward locomotion, move the left hand laterally to strafe, and move the right hand laterally to turn. For the first hand-only locomotion retest, prefer left-hand forward/back movement or move the right hand immediately after locomotion arms; reserve a separate stable right-hand pinch for the Trace row. If no movement occurs, keep the copied `captureSummary.locomotionProbeSummary` payload so engineering can see whether controller sources were absent, axes stayed below deadzone, pinch was not detected, joints were missing, dwell was still arming, gesture movement stayed below deadzone, or an active input vector failed to produce a rig delta.
