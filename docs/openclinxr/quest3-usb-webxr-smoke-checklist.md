...
Use `docs/openclinxr/quest-manual-performance-template.json` to capture the run, or save the copied in-app Quest Evidence payload containing `manualPerformanceDraft` plus `captureSummary` and pass that file to the same command. The checker validates `manualPerformanceDraft` with the raw report rules, preserves `captureSummary` blockers such as stale frame stats, and never treats `captureSummary.manualValidationReady` as sufficient by itself. The Frames row now splits total samples into `vr` and `preview` counts; a Full VR report must show `performance.immersiveFramesObserved` above zero, otherwise it is only proof of flat-preview rendering or an incomplete copy. Complete the operator, setup, immersive-session, explicit Full VR mode, foreground hand/controller input, locomotion, frame-sample, real controller-latency, battery, and comfort confirmations before validating the report. The Trace row should update after a completed in-headset controller or hand select, and the copied report should preserve `traceLatencyProxy.source: "xr_controller_select"` with the last trace tag and latency when that path is used. DOM trace-click evidence remains supporting evidence only and is marked as not a production controller-latency substitute. The checker remains the final authority and rejects malformed console-error arrays, missing or incorrect Full VR mode evidence, missing immersive frame counts after Full VR entry, missing hand/controller input, missing locomotion, sample windows larger than observed frames, implausible FPS values, nonpositive frame times, stale/unsampled copied frame stats, and out-of-range battery percentages. Mixed Reality observations must use a separate future `mixed_reality_passthrough` evidence lane rather than being mixed into this Full VR report. Until a completed report exists, the benchmark gate records `quest_manual_performance:missing_quest_manual_performance_report`.
...
- In-app Quest Evidence Frames row shows `vr` frame count increasing after Full VR entry; copied report should preserve `performance.immersiveFramesObserved > 0`.
- In-app Quest Evidence Trace row shows `xr_controller_select`, a known trace tag, and positive latency after a completed controller trigger or deliberate hand-select in Full VR.
- Console errors.
