...
- Thumbstick, keyboard, room-scale, or deliberate hand-gesture locomotion changes the rig position or turn angle without leaving the bounded bay; hand-only runs should record `activeLocomotionSource: "xr_hand_gesture"` and preserve `locomotionDelta` when that path is used.
...
- In-app Quest Evidence Trace row shows `xr_controller_select`, a known trace tag, measured timestamp, and positive latency after a completed controller trigger or deliberate hand-select in Full VR.
- Console errors.
