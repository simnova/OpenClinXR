...
Harvest before you dequeue. If any slice has a returned dispatch — a worker whose latest ledger row is `phase: completed` — deal with it before selecting new work, whether or not a worker is still running. A `ready_to_integrate` handoff whose worktree is not yet on main gets its contract re-verified and, only on green proofs, integrated; a `needs_resume` handoff is resumed or salvaged and is never integrated, because its planted RED is still unflipped; a worktree already on main whose card still reads `Dispatched` needs the board field repaired after the contract is verified against main, since git ancestry alone does not show the work is done. A returned dispatch also blocks its write root, so starting a new slice ahead of it risks two writers in one tree.
