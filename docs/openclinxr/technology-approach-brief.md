...
- Treat the [Ant Design CLI skill](https://github.com/ant-design/ant-design-cli/blob/main/skills/antd/SKILL.md) as optional offline component knowledge for Ant Design 6 work. If adopted, prefer a package-managed invocation over an untracked global install, use JSON output, query exact-version props/tokens/demos before writing components, and run AntD doctor checks after material admin UI changes; this workspace does not assume a global `antd` executable. Defer an AntD CLI lint gate until the local `pnpm dlx` `ERR_REQUIRE_ESM` failure is resolved.
- Use `@graphql-codegen/cli` with the client preset for route/workbench query documents and TypeScript resolver plugins for server contracts; avoid older Apollo-specific generated hooks until compatibility with the selected Apollo Client version is verified. The first generated slice now exports typed documents through `@openclinxr/graphql/client` and typed resolver contracts through `@openclinxr/graphql/resolvers`.
