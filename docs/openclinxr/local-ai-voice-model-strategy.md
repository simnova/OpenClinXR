...
Follow-up runtime evidence: `docs/openclinxr/api-python-backend-runtime-smoke-2026-05-05.json` proves the FastAPI backend can run in an ignored local venv and satisfy health plus JSON/binary WebSocket frame handling with canonical realtime voice protocol and latency-field proof. This clears the backend execution smoke, but live dialog remains blocked on real local inference, audio codec, Quest capture/playback, and safety controls.
