...
The refreshed 2026-05 probe records Bun and Portless as available, while Ollama and MLX LM remain missing from PATH. Historical local-model and local-voice benchmark artifacts have been pruned unless they are still compatibility inputs for current validation. Regenerate local model or voice evidence only for an active implementation slice; these smokes do not prove live clinical dialogue. Blender-backed placeholder asset baking has had passing local smoke evidence, but production avatar generation still needs rigging, LOD, texture, collider, and headset-performance evidence.
...
- Historical local model cache/source/runtime/quality artifacts are no longer retained as active evidence unless a validator still lists them as compatibility inputs. `pnpm local:model:runtime -- --parse-log <local-log> --output <report.json>` can harvest an existing local raw log without rerunning inference. The explicit `--execute-approved-local-run` mode can run only the approved local `Qwen3-4B-Q4_K_M.gguf` path with `llama.cpp --offline`; it writes raw logs outside the repo and commits evidence only when a current product slice needs it.
- Local voice candidate intake accepts only `microsoft/VibeVoice-Realtime-0.5B` (`src-vibevoice-github-2026`).
- Historical local voice, realtime-transport, and Python-backend smoke artifacts are no longer retained as active evidence by default. `pnpm local:voice:runtime -- --log <local-log> --prompt <local-text> --audio <local-wav> --output <report.json>` can harvest existing VibeVoice log/audio evidence without rerunning VibeVoice or committing generated audio; it does not make stale runtime evidence fresh or prove live dialog readiness.
