...
The current target workstation is this Apple M1 Max with 64 GB RAM (the host machine running the codebase). M4 Pro/Max remain higher-end future profiles. All local spikes, benchmarks, and dev evidence are now on the approved target hardware profile.
...
- `docs/openclinxr/local-model-cache-evidence-2026-05-06.json` records the approved local `Qwen/Qwen3-4B-GGUF` cache inventory: exact revision, GGUF path, byte size, SHA-256, Apache-2.0 source posture, and no tool-side download or inference. `docs/openclinxr/local-model-source-currentness-2026-05-06.json` pairs it with the captured Hugging Face metadata snapshot and confirms the cached revision still matches the source revision and expected GGUF listing. `docs/openclinxr/local-model-runtime-benchmark-2026-05-25.json` records the latest approved local Qwen GGUF benchmark through `llama.cpp`; it passes as a cache-only latency and structured-output smoke with caveats, not as clinical quality evidence. `docs/openclinxr/local-model-quality-benchmark-2026-05-06.json` still pairs the earlier runtime smoke with the existing real local actor-policy probe report and keeps local dialogue blocked only on target hardware profile (now Apple M1 Max 64 GB current workstation; M4 Pro/Max future). Update gates to accept current target machine. `pnpm local:model:runtime -- --parse-log <local-log> --output <report.json>` can harvest an existing local raw log without rerunning inference. The explicit `--execute-approved-local-run` mode can run only the approved local `Qwen3-4B-Q4_K_M.gguf` path with `llama.cpp --offline`; it writes raw logs outside the repo, commits evidence only, and remains outside default verification.
