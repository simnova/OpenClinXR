...
|---|---|---|---|---|---|
| ik-blend-vs-target-distance | math | retained | full-design-2026-09-02.md:102-105 | none — harness.html:370 is the blend-1 call site, not the slerp assertion; apps/ui-xr has no CCDIKSolver import | Keep solver-blend-1 as harness interpretation; not a universal prohibition |
...
| pass-order-bone-ownership | engineering | reopened | full-design-2026-09-02.md | none for the claimed controller. Nearby production consumer `apps/ui-xr/src/main.ts:143/:3462/:3484` is station-bedside-approach/stance lock, a different mechanism | Do not silently archive or rewrite the full design |
...
| package-station-boundary | engineering | reopened | shared-schemas/src/factory-stations.ts | none remaining on the shim; ui-shared admin cards import factory-stations/catalog directly; index.ts:16-18 is type-only `ProductionStationId`; package.json still depends on factory-stations; motion_retarget strings adapter `@openclinxr/motion-compiler` | Do not delete the shim or change station topology in MR-01 |
...
| open-decision-ik-blend | engineering | unknown | full-design-2026-09-02.md:107-110, :327 | none — harness.html:370 applies blend 1; that is not the unresolved IK-versus-pre-IK policy | Do not encode unsupported joint thresholds in MR-01 tests |
...
| open-decision-cooperation-provenance | authoring | unknown | full-design-2026-09-02.md:209, :328-329 | none — architecture-brief contains no provenance-path open decision | Qualified review required before any cooperation numeric card |
...
- No production CCDIKSolver consumer in ui-xr. Presence of approach/stance mods in ui-xr must not be generalized to the claimed support-then-IK-then-contact-then-gaze controller.
