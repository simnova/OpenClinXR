...
Approval manifest `../approvals/psr-01c.json` (332 rows: 83 keep, 247 remove, 2 migrate; `rawInventoryHash` `e43ee9c249ee7469828c39a265b9b1fdffc614a71768b5bbe419e5380bb70026`; `groupHash` `a8bd55837e0db542cf66dee57048b6dbac253f7eb6e175e15c6919bf59c89b24`). Machine-readable companion: `psr-03.json` in this directory.

## Commands and outcomes

- `pnpm arch:public-surface:verify -- --require-applied psr-03` - pass: 46 roots, 137 entrypoints, 2316 root symbols, 3339 occurrences, 2774 unique, 547 duplicated names (surface ce1644a6de04); ok: group psr-03 applied: 247 removed, 2 migrated; ok baseline ratchet: surface matches the checked-in baseline (no growth)
...
Before (compiler-derived on base): 46 roots, 137 entrypoints, 2,533 root symbols, 3,588 occurrences, 3,020 unique, 550 duplicated names. After (this tree): 46 roots, 137 entrypoints, 2,316 root symbols, 3,339 occurrences, 2,774 unique, 547 duplicated names. Delta: -217 root symbols, -249 occurrences, -246 unique, -3 duplicated names. Baseline ratchet: no growth. Group gate: 247 removed, 2 migrated.

## Dispositions per package

| Package | Keep | Remove | Migrate | Root after |
| --- | ---: | ---: | ---: | ---: |
| packages/openclinxr/data-mongodb | 0 | 73 | 0 | 0 |
| packages/openclinxr/motion-compiler | 7 | 76 | 0 | 0 |
| packages/openclinxr/conversation-policy | 27 | 32 | 0 | 27 |
| packages/openclinxr/model-gateway | 11 | 23 | 0 | 11 |
| packages/openclinxr/graphql | 38 | 43 | 2 | 9 |
...
## Removed symbols (247, from approvals/psr-01c.json)

Full exact list is in `psr-03.json` (`removed`). By package: data-mongodb 73 (entire root emptied to `export {}`; implementations stay reachable by deep relative path for intra-package tests); motion-compiler root 76 (root emptied to `export {}`; `./planted-red-manifest` keeps 7); conversation-policy 32; model-gateway 23; graphql root 11 + ./client 32.
...
- `packages/openclinxr/data-mongodb/src/exam-run-ledger.test.ts`: `./index.js` -> `./exam-run-ledger.js + ./exam-repositories.js` (createExamRunLedger, examRunLedgerClaimBoundary, examRunLedgerNotEvidenceFor, MemoryExamRunLedger, createMongoExamPersistence)
- `packages/openclinxr/data-mongodb/src/mongodb-repositories.integration.test.ts`: `./index.js` -> `./conversation-repositories.js + ./encounter-materialization-evidence-repositories.js + ./exam-repositories.js + ./faculty-repositories.js + ./persistence-sink.js + ./scenario-repositories.js + ./records.js` (createMongoApiPersistenceSink, createMongoDurableMultiActorSessionStore, MongoDurableClinicalEventRepository, MongoDurableConversationTurnRepository, MongoDurableEmotionalStateTimelineRepository, MongoEncounterMaterializationEvidenceRepository, MongoExamFormRepository, MongoRuntimeAssetBundleRepository, MongoStationRunQueueRepository, MongoFacultyScoreDraftRepository, MongoReviewPacketRepository, MongoScenarioRepository, MongoScenarioReviewDecisionRepository, MongoTraceRepository, durableActorTurnPersistenceScope, durableClinicalEventPersistenceScope, saveLearnerRuntimeAssetBundleFromGeneratedReport)
- `packages/openclinxr/data-mongodb/src/api-persistence-sink.integration.test.ts`: `./index.js` -> `./persistence-sink.js` (createMongoApiPersistenceSink)
- `packages/openclinxr/graphql/src/admin-graphql.test.ts`: `./index.js` -> `./documents.js + ./schema.js` (adminGraphqlDocumentByOperationName, adminGraphqlDocuments, buildAdminGraphqlSchema)
- `packages/openclinxr/graphql/src/the-faculty-disposition-contract-is-append-only.test.ts`: `./index.js` -> `./schema.js` (buildAdminGraphqlSchema)
- `packages/openclinxr/graphql/src/the-review-packet-replay-includes-actor-turn-layers.test.ts`: `./index.js` -> `./documents.js + ./schema.js` (adminGraphqlDocumentByOperationName, buildAdminGraphqlSchema)
- `packages/openclinxr/model-gateway/src/model-gateway.test.ts`: `./index.js (removed names)` -> `./actor-prompt.js + ./local-providers.js` (buildActorCommunicationProfilePromptContext, buildActorResponseProviderPromptInput, createMlxModelProviderAdapter, createLlamaCppModelProviderAdapter, createOllamaModelProviderAdapter)
...
- `packages/openclinxr/model-gateway/src/deepseek-actor-thinking-is-disabled.test.ts`: `./index.js (removed names)` -> `./openai-compatible-adapter.js + ./model-gateway-internal.js` (OpenAiCompatibleModelProviderAdapter)
- `packages/openclinxr/model-gateway/src/muse-spark-sends-no-reasoning.test.ts`: `./index.js (removed names)` -> `./openai-compatible-adapter.js + ./model-gateway-internal.js` (OpenAiCompatibleModelProviderAdapter)
- `packages/openclinxr/model-gateway/src/openai-compatible-adapter.test.ts`: `./index.js (removed names)` -> `./openai-compatible-adapter.js + ./model-gateway-internal.js` (OpenAiCompatibleModelProviderAdapter)
...
PSR-03 applies PSR-01C exactly: 247 root exports removed, 2 graphql document symbols migrated from root to the existing ./documents subpath, implementations kept, all in-repo consumers moved to kept routes, verify/build/typecheck green.
