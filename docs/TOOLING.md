...
**Turbo env:** `globalEnv` keeps `CI` + `NODE_ENV` only. Do not import ATL ICD/portless pass-through lists. Revisit task-level `env` only if wrong cache hits involve `OPENCLINXR_BUILD_*` / `VITE_*` in package builds.
