# PROTO_VERIFY_DELEGATION — incident archive (split 2026-08-29)

Cold storage. **Not loaded as project instructions.** Split out of
`agents/rules/PROTO_VERIFY_DELEGATION.md`, which was 284,792 characters and roughly 71,000 estimated
tokens resident on EVERY turn — about 70% of this repo's entire always-loaded instruction budget.

The split was not a judgement about value. The file's own frozen header, written 2026-08-23, made it:

> **This file is closed to additions.** Measured today: 4,456 lines, 46,540 words, 165 numbered
> headings. A document that size is an incident archive, not an execution protocol.
>
...
**Nothing here was deleted.** These are the 150 dated incident retrospectives, verbatim and in order.
They remain the record of why each rule exists, and they stay greppable — which is how the frozen
header says they should be read.

**What stayed loaded** in `agents/rules/PROTO_VERIFY_DELEGATION.md`: the operator directives table
(D1-D14), the D9 and D11 operational sections, the standing brief-template lines, the hard limits, the
eight core numbered rules, the fabrication tell, and the dispatch facts that cost real time.

**Consult by grep:** `grep -n '<term>' docs/_archive/agent-rules/2026-08/PROTO_VERIFY_DELEGATION-incident-archive.md`
...
## 9n. RESOLVED 2026-08-29 — the mechanism was built, and this section outlived its cause

**The heading below was true when written and is now false.** The `measured-before:` `done_when` rule
exists and does exactly what this section asks for. Measured today:

- Payload `measured-before:<artifact>:<product>` — `done-when-tree.js` header, `#177`.
- It reads a **trusted spawn-state baseline**, `SLICE_BASELINE_SCHEMA = "openclinxr.slice-baseline.v1"`,
  described in that file as *"the trusted spawn-state both `changed:` and `measured-before:` read"*.
  That is this section's own prescription — dispatch recording the tree state at spawn, and the proof
  comparing against it — implemented rather than promised.
- It is deliberately excluded from `writeBaselineHashes` target membership (`:128-131`), so the rule
  kind skips exact-membership while keeping every other fail-closed validation.

**Do not read `board-brief.ts:249` as invalidating it.** That correction removed `measured-before:`
from the *`it.fails` plant-protection* set, where it genuinely does not belong: it says nothing about
whether a plant was flipped, and only `live:` gives that guarantee. Being wrong as plant-protection is
not the same as being wrong as an ordering gate, and conflating the two is what left this section
stale for three days.

**So: a brief needing a pre-fix measurement uses `measured-before:`, not `exists:`.** An `exists:`
rule on a pre-fix artifact is the thing this section spent four workers complaining about, and it is
now a choice rather than a limitation.

**The durable lesson is `§10c`'s, arriving a second time.** This file records why each rule was
written and never records when the underlying defect was fixed. I re-read this section today, believed
it, and nearly hand-rolled around a built mechanism — the same shape as `D1`, and the reason the
curious-researcher protocol exists. When you close an issue a section here was written for, come back
and mark the section.

### The original section, retained as the planting record

## 9n-orig. The pre-fix gate is now SIX workers deep and it is still prose
