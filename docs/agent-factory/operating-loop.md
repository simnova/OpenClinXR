...
**Note (aligned to current AGENTS.md state):** This describes the *full iteration loop* (briefs, core plans, adversarial counterplans, leadership review, synthesis). Per current working model: use the full loop only for plateau recovery, major planning, broad drift review, or leadership synthesis. The daily driver for product advancement is OpenClaw-style continuous small deterministic slices (see `docs/openclinxr/openclaw-runbook-2026-05-27.md`, AGENTS.md, snapshots in `PROJECT_STATUS.md` + `worker-backlog-and-validation-matrix.md` for rehydration; HOT cards on the GitHub board; lease for unattended). Legacy ledgers `PROJECT_COORDINATION_INDEX.md` / `AUTONOMOUS_WORK_PLAN.md` remain as **archived audit stubs** (root + `docs/_archive/coordination/2026-08/`) — not living status. Live `iterations/` is empty except `iterations/README.md`; historical synthesis for 0009 is only `docs/_archive/iterations/0009/07-final-synthesis.md`. Artifact basenames in the loop sequence below are the process contract for a **new** iteration, not paths that must exist today. Re-read only snapshots for continuation; use `grep` tool + limits. After edits, run `pnpm agent:alignment && pnpm docs:drift-check`.
