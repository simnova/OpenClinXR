"""AO bake with max_ray_distance: junction-only contact darkening."""
import sys
import bpy
...
for mat_name, objs_ in by_mat.items():
    ...
    nt = mat.node_tree
    bsdf = find_bsdf(mat)
    img = bpy.data.images.new(f"ao_{int(ray_dist*100)}_{mat_name}", width=1024, height=1024, alpha=True)
    ...
    img.update()
    print(f"PROBE_AO3 dist={ray_dist} mat={mat_name} " + str(mapped_stats(img)))
