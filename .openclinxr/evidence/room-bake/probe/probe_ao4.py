"""AO bake with use_cage + max_ray_distance."""
import sys
import bpy
...
ray_dist = float(_argv_after_double_dash()[0])
...
def mapped_stats(img):
    ...
    if not vals:
        return {"mapped": 0}
    vals.sort()
    return {"mapped": len(vals), "min_p1": round(vals[int(len(vals)*0.01)],3), "p50": round(vals[int(len(vals)*0.50)],3), "p90": round(vals[int(len(vals)*0.90)],3), "max_p99": round(vals[int(len(vals)*0.99)],3), "mean": round(sum(vals)/len(vals),3)}
...
for mat_name, objs_ in by_mat.items():
    ...
    nt = mat.node_tree
    bsdf = find_bsdf(mat)
    img = bpy.data.images.new(f"ao4_{int(ray_dist*100)}_{mat_name}", width=1024, height=1024, alpha=True)
    ...
    img.update()
    print(f"PROBE_AO4 cage dist={ray_dist} mat={mat_name} " + str(mapped_stats(img)))
