"""AO bake probe: does bake_type=AO give strong contact darkening on the room?"""
import sys
import bpy
...
def mapped_stats(img):
    ...
    for i in range(n):
        r = pixels[i*4]
        if r > 0.01:  # mapped texels (unmapped stay black)
            vals.append(r)
    if not vals:
        return {"mapped": 0}
    vals.sort()
...
for mat_name, objs_ in by_mat.items():
    ...
    nt = mat.node_tree
    bsdf = find_bsdf(mat)
    img = bpy.data.images.new(f"ao_{mat_name}", width=1024, height=1024, alpha=True)
    ...
    img.update()
    print(f"PROBE_AO mat={mat_name} " + str(mapped_stats(img)))
