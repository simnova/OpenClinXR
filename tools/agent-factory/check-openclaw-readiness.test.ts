...
import { buildOpenClawReadinessReport } from "./check-openclaw-readiness";
...
describe("buildOpenClawReadinessReport", () => {
  ...
  it("fails when the worktree has uncommitted paths", () => {
    ...
    expect(report.failures.map((failure) => failure.check)).toContain("worktree-clean");
  });
  ...
  it("fails when the branch is behind upstream", () => {
    ...
    expect(report.ok).toBe(false);
    expect(report.failures.map((failure) => failure.check)).toContain("upstream-current");
  });
  ...
  it("fails when the branch has unpublished commits", () => {
    ...
    expect(report.ok).toBe(false);
    expect(report.failures.map((failure) => failure.check)).toContain("upstream-published");
  });
});
