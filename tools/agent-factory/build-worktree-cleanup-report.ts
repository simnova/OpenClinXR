...
const outputJson = ".agent-factory/worktree-cleanup-current.json";
...
const report = {
  ...
  claimBoundary: "local worktree cleanup cache only; not product readiness, clinical evidence, scoring evidence, Quest readiness, or durable repo evidence",
  usageRule: "Use this local cache report to keep cleanup commits separate from product-code and generated-code stabilization work; do not commit it as docs evidence.",
  counts,
  entries,
};
...
console.log(JSON.stringify({ outputJson, total: entries.length, counts }, null, 2));
