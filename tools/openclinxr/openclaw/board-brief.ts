...
export function boardProtocolSection(taskId: string | undefined): string[] {
  ...
  return [
    ...
    ``,
    `THE TOOL NAMES BELOW ARE LITERAL — copy them exactly, with no added prefix.`,
    `MEASURED 2026-09-14: the first worker to receive this block spent FOUR of its five calls`,
    `inventing prefixed variants of the bind tool before the plain registered name worked. Those`,
    `four turns bought nothing. The names written here are the registered ones; do not guess.`,
    ``,
    `AT THE START:`,
...
export function briefFromIssue(issue: BoardIssue, treeRoot?: string): BriefResult {
  ...
  return {
    ...
    prompt: [
      ...
      `from typecheck. Do not add a biome-ignore to pass this.`,
      ``,
      ...
      `promotion gate.`,
      ``,
      `RED planting convention: tools/openclinxr/evidence/ root is FROZEN at 560 .test.ts files.`,
      `A new root-level test file fails the architecture gate. Plant REDs in a SUBDIRECTORY`,
      `named for the subject (e.g. tools/openclinxr/evidence/<subject>/). The escape route is`,
      `always open — subdirectory test files are never counted.`,
      ``,
