...
it("old importer aliases delegate to actual admitted factory and real private clock/data helpers", () => {
  const facade = readFileSync(new URL("tools/openclinxr/factory/actor-audio-package-subpath-integration/prepared-actor-audio-compat.ts", root), "utf8");
  expect(facade).toMatch(/import\s*\{\s*createActorAudioRuntime\s*\}\s*from\s*["']@openclinxr\/xr-dialogue\/actor-audio-runtime["']/);
