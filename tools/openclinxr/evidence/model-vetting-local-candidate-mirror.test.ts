...
describe("model-vetting local candidate mirror", () => {
  ...
  it("skips unavailable generated-output candidates while mirroring available actors", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "openclinxr-local-candidate-mirror-skip-"));
    const cwd = process.cwd();
    try {
      process.chdir(tempDir);
      await writeFile("candidate.glb", "fixture-glb", "utf8");
      ...
      const preflight = {
        ...
        candidates: [
          baseCandidate,
          {
            ...
            actorMapping: {
              ...baseCandidate.actorMapping,
              actorId: "nurse_kevin_lee_v1",
              actorRole: "nurse",
            },
            paths: {
              ...baseCandidate.paths,
              sourceGlbPath: "missing-nurse.glb",
            },
          },
        ],
      };
      const manifest = await buildLocalCandidateModelVettingMirror({
        sourcePreflightPath: "preflight.json",
        sourcePreflight: preflight,
        outputHome: buildCagematchOutputHome("mirror-skip-test", "run"),
      });

      expect(manifest.mirroredCandidates).toHaveLength(1);
      expect(manifest.skippedCandidates).toEqual([
        {
          candidateId: "peds_nurse_kevin_anny_compatible_candidate",
          actorId: "nurse_kevin_lee_v1",
          sourceGlbPath: "missing-nurse.glb",
      ...
      const report = JSON.parse(await readFile(manifest.localModelVettingReportPath, "utf8")) as { candidates: Array<{ candidateId: string }> };
      expect(report.candidates.map((candidate) => candidate.candidateId)).toEqual(["peds_patient_child_anny_compatible_candidate"]);
    } finally {
      process.chdir(cwd);
      await rm(tempDir, { recursive: true, force: true });
    }
