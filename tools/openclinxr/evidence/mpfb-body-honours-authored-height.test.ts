 ...
 * ## FIXED (#329)
 *
 * The chain `buildActorPhenotypeExport -> macro dict -> apply_macros` is now joined.
 * body_param_stage.py gains `derive_macro_dict_from_authored_phenotype` (gender_presentation,
 * age-in-years, bmi and build all reach the macro dict; the other four macros keep a documented
 * neutral default when the case authors nothing for them) and `solve_height_macro_from_stature`,
 * which solves the height macro against MPFB's OWN exported body via the SAME bake-measure-
 * interpolate machinery #328 proved (imported lazily from materialize_mpfb_humanoid_candidate.py —
 * one solver, no re-implementation), refusing loudly when the target is outside the measured
 * band. The per-actor reachable band is measured at height macro 0 and 1 for each actor and
 * recorded. The solve artifact at .openclinxr/evidence/issue-329/phenotype-macro-solve.json was
 * generated by running those production functions over the committed actor-phenotype export:
 *
 *   actor                     | authored | solved height macro | measured | band (cm)
 *   patient_maya_johnson_v1   | 125 cm   | 0.6331              | 124.82   | 82.5 - 163.0
 *   parent_tara_johnson_v1    | 166 cm   | 0.5604              | 165.99   | 123.4 - 228.6
 *   nurse_kevin_lee_v1        | 176 cm   | 0.5385              | 175.56   | 136.9 - 242.6
 *
 * All three are within the +/-1 cm tolerance (MADR 0051 §5); the three bands are distinct (the
 * counterweight), so echoing a float or evaluating a formula could not have produced them.
 * "whether MPFB's band reaches 125 cm" is now MEASURED: it does (82.5-163.0 cm for this child's
 * age/gender/weight macros). The measured heights are on the MPFB2 create_human body (scale 0.1,
 * helpers stripped) — the same instrument #328 shipped the child at 124.1 cm with.
 *
 * NOT TESTED: the body-param CLI has NOT been rewired to feed authoredPhenotype into the two adult
 * body classes. The stage path does that when the body class carries it, and the chain is proven
 * by this artifact, but shipping the two adults at their authored heights additionally requires
 * the measured import-path fix (the stage's import_obj body does not respond to the height macro
 * because the OBJ importer's 90-degree object rotation is not baked before macro targets apply —
 * probed 2026-08-11, band ~17.3-17.5 dm regardless of height macro; baking the rotation restores
 * the full band). That is a separate re-bake slice.
 */
