 ...
 * ## FIXED (#726)
 *
 * (1) flipped `it.fails` -> `it` on 2026-08-27 after the capture recorded `sources` (producer +
 * subject GLB) with per-source `fingerprints` (bytes + sha256, #707's helper reused, D1) and
 * re-pinned the subject by actor id — `parent_tara_johnson_v1`, stamped on the humanoid root
 * (ui-xr-viseme-drive-capture.ts EXPECTED_SUBJECT_ACTOR_ID) — instead of first-viseme-in-traversal,
 * which the child now beats. The subject GLB path is read live from the framed root's
 * `openClinXrAssetPath`, never typed.
 *
 * Re-run 2026-08-27 (same tree the test runs against): aimJointName=jaw, aimWorldY=1.1024,
 * crownApexWorldY=1.2518 (drop 0.1495 > the older contract's 0.08 band — the parent now renders
 * SEATED per #574, so the numbers are not the 2026-08-20 standing adult's), subjectVisible=true,
 * firstHitMeshName=mpfb_ob_patient_aisha_body_1 (not the stretcher rail), subjectInFrame=true,
 * headNdc≈(0,0). Drive unchanged: 5 distinct strong visemes at influence 1.0 across 88 live
 * samples. The three sibling summaries (parent-drives-a-real-viseme, reframe-subject-in-frame,
 * review-panel-leaves-exam-volume) were regenerated by the same run and describe the same subject.
 */
...
describe("the mouth capture artifact answers for the current tree (#726)", () => {
   ...
   */
  it("(1) the artifact declares what it measured, and it still matches the tree", () => {
    const doc = mouth();
