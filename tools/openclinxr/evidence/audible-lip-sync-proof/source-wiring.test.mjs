...
it('the real product host imports prepared audio source from its app-local module',()=>{
 expect(main()).toMatch(/import\s*\{[^}]*startPreparedActorTurnAudio[^}]*\}\s*from\s*["']\.\/prepared-actor-audio\.js["']/);
});
it('the real speak handshake acknowledges the shared source manager instead of unconditional true',()=>{
...
it('actual displayed frame snapshots source clock immediately before existing sole animation writer',()=>{
...
it('the existing dev dialogue ingress and preparation bridge are wired rather than a second renderer-only path',()=>{
...
it('unprepared product source manager returns false instead of acknowledging voice',()=>{
 expect(startPreparedActorTurnAudio({actorId:'clock-patient',spokenText:'clock fixture'})).toBe(false);
});
it('caller running-context labels cannot grant an unprepared product voice ACK',()=>{
