...
describe("actual source-owned audible playback position", () => {
  ...
  it("scheduled source start connects real adapter and never advances before its audio start", async () => {
  ...
  it("source offset and playback rate select sample position instead of wall elapsed time", async () => {
  ...
  it("actor pause holds source position and resumes with a new one-shot node", async () => {
  ...
  it("late end callback of a paused node cannot end the resumed actor generation", async () => {
  ...
  it("suspended or rejected source start cannot report playing or animate speech", async () => {
  ...
  it("stop must be present and not false before the actor is reported stopped", async () => {
    const f = fixture(); await f.player.start({ when: 10, offset: 0, rate: 1 });
  ...
  it("independent actors sharing one context never pause or cancel each other's sources", async () => {
