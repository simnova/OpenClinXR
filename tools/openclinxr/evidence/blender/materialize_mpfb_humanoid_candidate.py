...
def _anny_manifest_for(manifest_id):
    ...
    Shipped references live beside their .anny_base.obj under generated-humanoids.
    A case-authored EVIDENCE staged reference (generated by generate_mesh.py for
    exactly one actor's phenotype, tracked beside the issue's artifacts) carries
    the same schema and is found in .openclinxr/evidence/issue-651/. None when no
    manifest exists — callers keep their documented defaults.
    """
    shipped = REPO_ROOT / "apps/ui-xr/public/generated-humanoids" / f"{manifest_id}.anny_manifest.json"
    if shipped.is_file():
        return shipped
    staged = REPO_ROOT / ".openclinxr/evidence/issue-651" / f"{manifest_id}.anny_manifest.json"
...
def phenotype_numeric_block(manifest_id):
    ...
    if not manifest_id:
        return {}
    manifest = _anny_manifest_for(manifest_id)
    if manifest is None:
        return {}
    try:
        m = json.loads(manifest.read_text(encoding="utf-8"))
...
EYE_DIAMETER_TARGET_MM = {
    ...
    # #651 — the ED patient's own 178 cm adult-male body (case-driven numeric
    # identity bake); adult axial length applies.
    "mpfb-gown-adult-patient": 24.0,
}
