...
import {
  SC09_FROZEN_SCOPES,
  SC09_REQUIRED_CHECK_IDS,
  SC09_REQUIRED_CONTROL_IDS,
  auditScopes,
  resolveArtifactPath,
  sha256Hex,
  verifyReport,
...
function goodReport(): Record<string, unknown> {
  return {
    ...
    implementation: {
      ...
      changedFiles: ["tools/openclinxr/evidence/scene-closure/proofs/sc-09/a.ts","tools/openclinxr/evidence/scene-closure/the-final-acceptance-rejects-incomplete-evidence.test.ts"],
    ...
    counterweight: {
      testIds: ["SC-09-required-behavior"],
    ...
    checks: SC09_REQUIRED_CHECK_IDS.map((checkId) => ({
      checkId,
    ...
    controls: SC09_REQUIRED_CONTROL_IDS.map((controlId) => ({
      controlId,
      trigger: "constructed",
      expected: "refusal",
    ...
    artifacts: [
      {
        ...
        objectKey: "sc-09/observations.jsonl",
...
const OBJECTS = {
  "/store/sc-09/observations.jsonl": ARTIFACT_BYTES,
  "/store/sc-09/baseline.txt": BASELINE_BYTES,
  "/store/sc-09/fixed.txt": FIXED_BYTES,
};

function verify(report: Record<string, unknown>, objects: Record<string, Buffer> = OBJECTS, links: Record<string, string> = {}) {
  return verifyReport({
    report,
    suppliedScopes: [...SC09_FROZEN_SCOPES],
    registry: REGISTRY,
...
describe("the SC-09 evidence verifier accepts a complete control and rejects everything else", () => {
  ...
  it("(3) CHANGED artifact bytes are rejected on the hash, not merely on presence", () => {
    const result = verify(goodReport(), {
      ...OBJECTS,
      "/store/sc-09/observations.jsonl": Buffer.from("tampered\n"),
    });
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it("(5) a SYMLINK escape out of the alias root is refused even though the key looks clean", () => {
    ...
    const links = { "/store/sc-09/observations.jsonl": "/elsewhere/observations.jsonl" };
  ...
  it("(14) the scope audit rejects an omitted, extra or duplicated scope", () => {
    expect(auditScopes(SC09_FROZEN_SCOPES)).toEqual([]);
    expect(auditScopes(SC09_FROZEN_SCOPES.slice(1))).toContain(`omitted --scope ${SC09_FROZEN_SCOPES[0]}`);
    expect(auditScopes([...SC09_FROZEN_SCOPES, "packages/openclinxr/telemetry"]))
      .toContain("extra --scope packages/openclinxr/telemetry");
    expect(auditScopes([...SC09_FROZEN_SCOPES, SC09_FROZEN_SCOPES[0]!]).join("\n")).toMatch(/duplicate/u);
  });
