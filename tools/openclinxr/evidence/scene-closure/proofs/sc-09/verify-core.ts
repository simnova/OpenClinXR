...
/** Every check the pinned SC-09 contract requires. A report cannot shrink this set. */
export const SC09_REQUIRED_CHECK_IDS = [
  "a01-through-a12-each-have-direct-evidence",
  "normal-activation-independently-exercised",
  "replay-independently-reopened",
  "website-playback-independently-watched",
  "upstream-report-hashes-unchanged-at-reviewed-head",
  "decisive-controls-rerun",
  "no-row-passes-from-child-status",
  "remaining-boundaries-stated-not-implemented",
] as const;

/** Every negative or known-good control the pinned SC-09 contract requires. */
export const SC09_REQUIRED_CONTROL_IDS = [
  "all-children-landed-but-missing-run-fails",
  "site-strings-green-with-failed-playback-fails",
  "api-bypass-despite-helper-tests-fails",
  "wrong-run-acknowledgment-fails",
  "replaced-support-fails",
  "absent-physician-fails",
  "stale-plan-fails",
  "corrupt-replay-fails",
  "source-edit-mismatch-fails",
] as const;

/** The card's frozen write roots, in the board's order. The CLI compares --scope against this. */
export const SC09_FROZEN_SCOPES = [
...
export const SC09_A_ROWS = ["A12"] as const;
...
export function auditScopes(
  ...
  frozen: readonly string[] = SC09_FROZEN_SCOPES,
): string[] {
...
export function verifyReport(input: VerifyInput): VerifyResult {
  ...
  if (report.cardKey !== "SC-09") fail(`wrong cardKey ${String(report.cardKey)}`);
  ...
  if (!isRecord(contract)) fail("missing contract section");
    ...
    for (const row of SC09_A_ROWS) if (!aRows.includes(row)) fail(`contract.aRows omits ${row}`);
  }
  ...
  if (!isRecord(implementation)) fail("missing implementation section");
    ...
    if (!Array.isArray(changedFiles) || changedFiles.length === 0) {
      ...
      for (const entry of changedFiles) {
        ...
        const inScope = SC09_FROZEN_SCOPES.some((scope) => {
  ...
  for (const check of checks) {
    ...
    if (!(SC09_REQUIRED_CHECK_IDS as readonly string[]).includes(checkId)) {
  ...
  for (const required of SC09_REQUIRED_CHECK_IDS) {
  ...
  for (const control of controls) {
    ...
    if (!(SC09_REQUIRED_CONTROL_IDS as readonly string[]).includes(controlId)) {
  ...
  for (const required of SC09_REQUIRED_CONTROL_IDS) {
