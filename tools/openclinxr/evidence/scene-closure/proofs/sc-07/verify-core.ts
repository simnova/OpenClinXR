...
/** Every check the pinned SC-07 contract requires. A report cannot shrink this set. */
export const SC07_REQUIRED_CHECK_IDS = [
  "recorder-starts-before-activation",
  "uninterrupted-through-arrival-and-stop",
  "only-normal-inputs-and-read-only-telemetry",
  "run-identity-shared-by-video-and-measurements",
  "source-hashes-bound-to-recording",
  "capture-gaps-recorded",
  "m1-runtime-facts-recorded",
  "sc00-rubric-applied-to-displayed-samples",
  "independent-visual-review-recorded",
  "originals-retained-and-independently-retrieved",
] as const;

/** Every negative or known-good control the pinned SC-07 contract requires. */
export const SC07_REQUIRED_CONTROL_IDS = [
  "wrong-actor-fails",
  "no-skinned-body-fails",
  "no-contacts-fails",
  "nonmonotonic-timestamps-fail",
  "flag-only-motion-fails",
  "stale-source-hash-fails",
  "corrupted-recording-fails",
  "review-only-handoff-fails",
  "injected-movement-fails",
  "unsupported-required-state-negative-run",
  "blocked-route-negative-run",
] as const;

/** The card's frozen write roots, in the board's order. The CLI compares --scope against this. */
export const SC07_FROZEN_SCOPES = [
...
export const SC07_A_ROWS = ["A08", "A10"] as const;
...
export function auditScopes(
  ...
  frozen: readonly string[] = SC07_FROZEN_SCOPES,
): string[] {
...
export function verifyReport(input: VerifyInput): VerifyResult {
  ...
  if (report.cardKey !== "SC-07") fail(`wrong cardKey ${String(report.cardKey)}`);
  ...
  if (!isRecord(contract)) fail("missing contract section");
    ...
    for (const row of SC07_A_ROWS) if (!aRows.includes(row)) fail(`contract.aRows omits ${row}`);
  }
  ...
  if (!isRecord(implementation)) fail("missing implementation section");
    ...
    if (!Array.isArray(changedFiles) || changedFiles.length === 0) {
      ...
      for (const entry of changedFiles) {
        ...
        const inScope = SC07_FROZEN_SCOPES.some((scope) => {
  ...
  for (const check of checks) {
    ...
    if (!(SC07_REQUIRED_CHECK_IDS as readonly string[]).includes(checkId)) {
  ...
  for (const required of SC07_REQUIRED_CHECK_IDS) {
  ...
  for (const control of controls) {
    ...
    if (!(SC07_REQUIRED_CONTROL_IDS as readonly string[]).includes(controlId)) {
  ...
  for (const required of SC07_REQUIRED_CONTROL_IDS) {
  ...
  for (const problem of inspectClosureEvidence(input.report, retainedObjects, SC07_FROZEN_SCOPES, input.inspection)) fail(problem);
