import { REQUIRED_SOURCE_PATHS as SC08_PATHS } from "../sc-08/closure-inspection.js";
import { REQUIRED_SOURCE_PATHS as SC09_PATHS } from "../sc-09/closure-inspection.js";
import { REQUIRED_SOURCE_PATHS as SC07_PATHS } from "./closure-inspection.js";
...
describe("bounded source and retained recording inspection", () => {
  it("keeps producer/runtime input policy identical across SC-07–09", () => {
    expect(SC07_PATHS).toEqual(SC08_PATHS);
    expect(SC07_PATHS).toEqual(SC09_PATHS);
    expect(SC07_PATHS).toContain("apps/ui-xr/src/main.ts");
    expect(
      SC07_PATHS.some(
        (name) => name.startsWith("docs/") || name.includes("proofs/sc-09"),
      ),
    ).toBe(false);
  });
  ...
  it("rejects NaN/absent watch duration and unbound source files inside otherwise owned roots", () => {
    ...
    const c = unitControl();
    c.inputs.push({
      path: "tools/openclinxr/evidence/scene-closure/proofs/sc-07/undeclared.ts",
      sha256: "0".repeat(64),
    });
