...
const BEHAVIOR_TEST_SOURCE = [
  ...
  `  it("${SC02_BEHAVIOR_TEST_TITLE}", async () => { expect(1).toBe(1); });`,
  "});",
  "",
].join("\n");
const CHANGED_SOURCE = "export const x = 1;\n";
const SOURCE_TREE: Record<string, string> = {
  [SC02_BEHAVIOR_TEST_PATH]: BEHAVIOR_TEST_SOURCE,
  "packages/openclinxr/shared-schemas/a.ts": CHANGED_SOURCE,
...
function goodReport(): Record<string, unknown> {
  return {
    ...
    implementation: {
      ...
      treeClean: true,
      inputs: [
        { path: SC02_BEHAVIOR_TEST_PATH, sha256: sha256Hex(BEHAVIOR_TEST_SOURCE) },
      ...
      changedFiles: ["packages/openclinxr/shared-schemas/a.ts","packages/openclinxr/scenario-runtime/a.ts"],
      runtime: { node: "v24", platform: "darwin-arm64" },
    },
    execution: {
      taskId: "tsk_96b5bb3346c0d96f",
      runId: "run-1",
      commands: SC02_REQUIRED_COMMANDS.map((command) => ({
        argv: command.split(" "),
        exitCode: 0,
    ...
    counterweight: {
      testIds: ["SC-02-required-behavior"],
    ...
    sourceInspection: {
      behaviorTestPath: SC02_BEHAVIOR_TEST_PATH,
      behaviorTestTitle: SC02_BEHAVIOR_TEST_TITLE,
    },
    encounter: { caseId: "scene_closure_supine_bedside_v1", caseVersion: 1 },
    observations: [
      {
    ...
    artifacts: [
      {
        ...
        objectKey: "sc-02/observations.jsonl",
...
const OBJECTS = {
  "/store/sc-02/observations.jsonl": ARTIFACT_BYTES,
  "/store/sc-02/baseline.txt": BASELINE_BYTES,
  "/store/sc-02/fixed.txt": FIXED_BYTES,
};
...
function verify(
  ...
  return verifyReport({
    report,
    suppliedScopes: [...SC02_FROZEN_SCOPES],
    registry: REGISTRY,
...
describe("the SC-02 evidence verifier accepts a complete control and rejects everything else", () => {
  ...
  it("(3) CHANGED artifact bytes are rejected on the hash, not merely on presence", () => {
    const result = verify(goodReport(), {
      ...OBJECTS,
      "/store/sc-02/observations.jsonl": Buffer.from("tampered\n"),
    });
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it("(5) a SYMLINK escape out of the alias root is refused even though the key looks clean", () => {
    ...
    const links = { "/store/sc-02/observations.jsonl": "/elsewhere/observations.jsonl" };
  ...
  it("(14) the scope audit rejects an omitted, extra or duplicated scope", () => {
    expect(auditScopes(SC02_FROZEN_SCOPES)).toEqual([]);
    expect(auditScopes(SC02_FROZEN_SCOPES.slice(1))).toContain(`omitted --scope ${SC02_FROZEN_SCOPES[0]}`);
    expect(auditScopes([...SC02_FROZEN_SCOPES, "packages/openclinxr/telemetry"]))
      .toContain("extra --scope packages/openclinxr/telemetry");
    expect(auditScopes([...SC02_FROZEN_SCOPES, SC02_FROZEN_SCOPES[0]!]).join("\n")).toMatch(/duplicate/u);
  });
  ...
  it("(18) CORRUPT SOURCE control: an input whose tree bytes no longer match its recorded hash fails", () => {
    const result = verify(goodReport(), OBJECTS, {}, {
      ...SOURCE_TREE,
      "packages/openclinxr/scenario-runtime/a.ts": "export const x = 2;\n",
    });
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it("(19) MALFORMED BEHAVIOR TEST control: skipped, marked, or comment-only titles are refused", () => {
    ...
    expect(inspectBehaviorTestSource(BEHAVIOR_TEST_SOURCE, SC02_BEHAVIOR_TEST_TITLE)).toEqual([]);
    for (const bad of [
      `describe("s", () => { it.skip("${SC02_BEHAVIOR_TEST_TITLE}", () => {}); it("${SC02_BEHAVIOR_TEST_TITLE}", () => {}); });`,
      `describe.skip("s", () => { it("${SC02_BEHAVIOR_TEST_TITLE}", () => {}); });`,
      `// it("${SC02_BEHAVIOR_TEST_TITLE}", () => {});`,
      `/* it("${SC02_BEHAVIOR_TEST_TITLE}", () => {}); */`,
      `describe("s", () => { it.fails("${SC02_BEHAVIOR_TEST_TITLE}", () => {}); });`,
    ]) {
      expect(inspectBehaviorTestSource(bad, SC02_BEHAVIOR_TEST_TITLE).length, bad).toBeGreaterThan(0);
    }
    // And through the whole verifier, with the tree holding a skipped test.
    const tree = {
      ...SOURCE_TREE,
      [SC02_BEHAVIOR_TEST_PATH]: `describe.skip("s", () => { it("${SC02_BEHAVIOR_TEST_TITLE}", () => {}); });`,
    };
    const report = goodReport();
    (report["implementation"] as Record<string, unknown>)["inputs"] = [
      { path: SC02_BEHAVIOR_TEST_PATH, sha256: sha256Hex(tree[SC02_BEHAVIOR_TEST_PATH]!) },
      { path: "packages/openclinxr/shared-schemas/a.ts", sha256: sha256Hex(CHANGED_SOURCE) },
      { path: "packages/openclinxr/scenario-runtime/a.ts", sha256: sha256Hex(CHANGED_SOURCE) },
    ];
  ...
  it("(20) a check citing a real, correctly hashed artifact that never mentions it is refused", () => {
    ...
    const result = verify(goodReport(), { ...OBJECTS, "/store/sc-02/observations.jsonl": bytes });
    expect(result.ok).toBe(false);
    if (result.ok) return;
    ...
    const mentionOnly = verify(report, { ...OBJECTS, "/store/sc-02/observations.jsonl": bytes });
