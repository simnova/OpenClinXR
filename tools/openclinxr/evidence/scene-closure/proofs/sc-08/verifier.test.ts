...
import {
  ...
  resolveArtifactPath,
  SC08_FROZEN_SCOPES,
  SC08_REQUIRED_CHECK_IDS,
  SC08_REQUIRED_CONTROL_IDS,
  sha256Hex,
  verifyReport,
} from "./verify-core.js";
...
function goodReport(): Record<string, unknown> {
  return {
    ...
    implementation: {
      ...
      changedFiles: ["docs/index.html/a.ts","docs/styles.css/a.ts"],
    ...
    counterweight: {
      testIds: ["SC-08-required-behavior"],
    ...
    checks: SC08_REQUIRED_CHECK_IDS.map((checkId) => ({
      checkId,
    ...
    controls: SC08_REQUIRED_CONTROL_IDS.map((controlId) => ({
      controlId,
      trigger: "constructed",
      expected: "refusal",
    ...
    artifacts: [
      {
        ...
        objectKey: "sc-08/observations.jsonl",
...
describe("the SC-08 evidence verifier accepts a complete control and rejects everything else", () => {
  ...
  it("(3) CHANGED artifact bytes are rejected on the hash, not merely on presence", () => {
    const result = verify(goodReport(), {
      ...OBJECTS,
      "/store/sc-08/observations.jsonl": Buffer.from("tampered\n"),
    });
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it("(5) a SYMLINK escape out of the alias root is refused even though the key looks clean", () => {
    ...
    const links = { "/store/sc-08/observations.jsonl": "/elsewhere/observations.jsonl" };
  ...
  it("(14) the scope audit rejects an omitted, extra or duplicated scope", () => {
    expect(auditScopes(SC08_FROZEN_SCOPES)).toEqual([]);
    expect(auditScopes(SC08_FROZEN_SCOPES.slice(1))).toContain(`omitted --scope ${SC08_FROZEN_SCOPES[0]}`);
    expect(auditScopes([...SC08_FROZEN_SCOPES, "packages/openclinxr/telemetry"]))
      .toContain("extra --scope packages/openclinxr/telemetry");
    expect(auditScopes([...SC08_FROZEN_SCOPES, SC08_FROZEN_SCOPES[0]!]).join("\n")).toMatch(/duplicate/u);
  });
