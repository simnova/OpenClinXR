...
/** Every check the pinned SC-01 contract requires. A report cannot shrink this set. */
export const SC01_REQUIRED_CHECK_IDS = [
  "persisted-case-reaches-compiled-bundle",
  "authored-version-change-reaches-consumer",
  "actor-identities-preserved",
  "supine-support-and-frame-authored",
  "physician-start-differs-from-destination",
  "required-binding-carries-source-rule",
  "absent-equipment-not-injected",
  "duplicate-copies-keep-distinct-ids",
  "loaded-scene-matches-persisted-case",
] as const;

/** Every negative or known-good control the pinned SC-01 contract requires. */
export const SC01_REQUIRED_CONTROL_IDS = [
  "unknown-binding-refuses",
  "reordered-assets-keep-binding",
  "absent-ecg-iv-not-inserted",
  "two-copies-do-not-collapse",
  "stale-fixture-cannot-override-persisted",
  "misidentified-physician-refused",
  "original-ward-case-unchanged",
] as const;

/** The card's frozen write roots, in the board's order. The CLI compares --scope against this. */
export const SC01_FROZEN_SCOPES = [
...
export const SC01_A_ROWS = ["A01"] as const;
...
export function auditScopes(
  ...
  frozen: readonly string[] = SC01_FROZEN_SCOPES,
): string[] {
...
export function verifyReport(input: VerifyInput): VerifyResult {
  ...
  if (report.cardKey !== "SC-01") fail(`wrong cardKey ${String(report.cardKey)}`);
  ...
  if (!isRecord(contract)) fail("missing contract section");
    ...
    for (const row of SC01_A_ROWS) if (!aRows.includes(row)) fail(`contract.aRows omits ${row}`);
  }
  ...
  if (!isRecord(implementation)) fail("missing implementation section");
    ...
    if (!Array.isArray(changedFiles) || changedFiles.length === 0) {
      ...
      for (const entry of changedFiles) {
        ...
        const inScope = SC01_FROZEN_SCOPES.some((scope) => {
  ...
  for (const check of checks) {
    ...
    if (!(SC01_REQUIRED_CHECK_IDS as readonly string[]).includes(checkId)) {
  ...
  for (const required of SC01_REQUIRED_CHECK_IDS) {
  ...
  for (const control of controls) {
    ...
    if (!(SC01_REQUIRED_CONTROL_IDS as readonly string[]).includes(controlId)) {
  ...
  for (const required of SC01_REQUIRED_CONTROL_IDS) {
