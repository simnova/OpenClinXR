...
import {
  SC01_FROZEN_SCOPES,
  SC01_REQUIRED_CHECK_IDS,
  SC01_REQUIRED_CONTROL_IDS,
  auditScopes,
  resolveArtifactPath,
  sha256Hex,
  verifyReport,
...
function goodReport(): Record<string, unknown> {
  return {
    ...
    counterweight: {
      testIds: ["SC-01-required-behavior"],
    ...
    checks: SC01_REQUIRED_CHECK_IDS.map((checkId) => ({
      checkId,
    ...
    controls: SC01_REQUIRED_CONTROL_IDS.map((controlId) => ({
      controlId,
      trigger: "constructed",
      expected: "refusal",
    ...
    artifacts: [
      {
        ...
        objectKey: "sc-01/observations.jsonl",
...
const OBJECTS = {
  "/store/sc-01/observations.jsonl": ARTIFACT_BYTES,
  "/store/sc-01/baseline.txt": BASELINE_BYTES,
  "/store/sc-01/fixed.txt": FIXED_BYTES,
};

function verify(report: Record<string, unknown>, objects: Record<string, Buffer> = OBJECTS, links: Record<string, string> = {}) {
  return verifyReport({
    report,
    suppliedScopes: [...SC01_FROZEN_SCOPES],
    registry: REGISTRY,
...
describe("the SC-01 evidence verifier accepts a complete control and rejects everything else", () => {
  ...
  it("(3) CHANGED artifact bytes are rejected on the hash, not merely on presence", () => {
    const result = verify(goodReport(), {
      ...OBJECTS,
      "/store/sc-01/observations.jsonl": Buffer.from("tampered\n"),
    });
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it("(5) a SYMLINK escape out of the alias root is refused even though the key looks clean", () => {
    ...
    const links = { "/store/sc-01/observations.jsonl": "/elsewhere/observations.jsonl" };
  ...
  it("(14) the scope audit rejects an omitted, extra or duplicated scope", () => {
    expect(auditScopes(SC01_FROZEN_SCOPES)).toEqual([]);
    expect(auditScopes(SC01_FROZEN_SCOPES.slice(1))).toContain(`omitted --scope ${SC01_FROZEN_SCOPES[0]}`);
    expect(auditScopes([...SC01_FROZEN_SCOPES, "packages/openclinxr/domain"]))
      .toContain("extra --scope packages/openclinxr/domain");
    expect(auditScopes([...SC01_FROZEN_SCOPES, SC01_FROZEN_SCOPES[0]!]).join("\n")).toMatch(/duplicate/u);
  });
