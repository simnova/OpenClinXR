...
/** Every check the pinned SC-03 contract requires. A report cannot shrink this set. */
export const SC03_REQUIRED_CHECK_IDS = [
  "caller-supplies-support-id-and-readiness",
  "delayed-mount-stays-pending",
  "wrong-instance-cannot-substitute",
  "invalid-normal-offset-refuses-acceptance",
  "offset-composed-in-orthonormal-frame-no-double-scale",
  "authored-offset-measured-on-posed-skinned-patient",
  "heading-survives-slot-repair",
  "persisted-case-intent-beats-fixture-fallback",
  "missing-physician-reported-not-substituted",
  "mount-replacement-invalidates-observation",
] as const;

/** Every negative or known-good control the pinned SC-03 contract requires. */
export const SC03_REQUIRED_CONTROL_IDS = [
  "slow-support-stays-pending",
  "same-kind-wrong-support-refuses",
  "post-acceptance-replacement-invalidates",
  "invalid-normal-offset-refuses",
  "malformed-frame-refuses",
  "changed-glb-scale-refuses",
  "slot-kind-repair-preserves-heading",
  "unauthored-supine-control-unchanged",
] as const;

/** The card's frozen write roots, in the board's order. The CLI compares --scope against this. */
export const SC03_FROZEN_SCOPES = [
...
export const SC03_A_ROWS = ["A04", "A05"] as const;

/** The named behavior test and its required ordinary `it` title, both fixed by the card. */
export const SC03_BEHAVIOR_TEST_PATH = "apps/ui-xr/src/the-mounted-support-controls-the-posed-patient.test.ts";
export const SC03_BEHAVIOR_TEST_TITLE = "SC-03-required-behavior";
 ...
 */
export const SC03_REQUIRED_COMMANDS = [
  ...
  `pnpm exec vitest run ${SC03_BEHAVIOR_TEST_PATH}`,
  `pnpm exec tsx tools/openclinxr/openclaw/assert-contract-live.ts ${SC03_BEHAVIOR_TEST_PATH} ${SC03_BEHAVIOR_TEST_TITLE}`,
  "pnpm exec vitest run tools/openclinxr/evidence/scene-closure/proofs/sc-03/verifier.test.ts",
] as const;
...
export function auditScopes(
  ...
  frozen: readonly string[] = SC03_FROZEN_SCOPES,
): string[] {
  const problems: string[] = [];
...
export function verifyReport(input: VerifyInput): VerifyResult {
  ...
  if (report.cardKey !== "SC-03") fail(`wrong cardKey ${String(report.cardKey)}`);
  ...
  if (!isRecord(contract)) fail("missing contract section");
    ...
    for (const row of SC03_A_ROWS) if (!aRows.includes(row)) fail(`contract.aRows omits ${row}`);
  }
  ...
  if (!isRecord(implementation)) fail("missing implementation section");
    ...
    if (!Array.isArray(changedFiles) || changedFiles.length === 0) {
      ...
      for (const entry of changedFiles) {
        ...
        const inScope = SC03_FROZEN_SCOPES.some((scope) => {
  ...
  if (!isRecord(sourceInspection)) fail("missing sourceInspection section");
    ...
    if (sourceInspection["behaviorTestPath"] !== SC03_BEHAVIOR_TEST_PATH) {
      fail(`sourceInspection.behaviorTestPath must be ${SC03_BEHAVIOR_TEST_PATH}`);
    }
    if (sourceInspection["behaviorTestTitle"] !== SC03_BEHAVIOR_TEST_TITLE) {
      fail(`sourceInspection.behaviorTestTitle must be ${SC03_BEHAVIOR_TEST_TITLE}`);
    }
    const source = input.sourceReader(SC03_BEHAVIOR_TEST_PATH);
    if (source instanceof Error) fail(`behavior test unreadable: ${source.message}`);
    else for (const problem of inspectBehaviorTestSource(source, SC03_BEHAVIOR_TEST_TITLE)) fail(problem);
  }
  ...
  if (!isRecord(execution)) fail("missing execution section");
    ...
    for (const required of SC03_REQUIRED_COMMANDS) {
  ...
  for (const check of checks) {
    ...
    if (!(SC03_REQUIRED_CHECK_IDS as readonly string[]).includes(checkId)) {
  ...
  for (const required of SC03_REQUIRED_CHECK_IDS) {
  ...
  for (const control of controls) {
    ...
    if (!(SC03_REQUIRED_CONTROL_IDS as readonly string[]).includes(controlId)) {
  ...
  for (const required of SC03_REQUIRED_CONTROL_IDS) {
