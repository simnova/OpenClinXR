...
/** Every check the pinned SC-01S contract requires. A report cannot shrink this set. */
export const SC01S_REQUIRED_CHECK_IDS = [
  ...
  "client-carries-selected-scenario-id",
  "catalog-source-reported",
] as const;

/** Every negative or known-good control the pinned SC-01S contract requires. */
export const SC01S_REQUIRED_CONTROL_IDS = [
  ...
  "equipment-survives-an-injected-document",
] as const;

/** The card's frozen write roots, in the board's order. The CLI compares --scope against this. */
export const SC01S_FROZEN_SCOPES = [
  "packages/openclinxr/rest/src/routes/runtime-evidence-routes.ts",
  "packages/openclinxr/xr-station/src/api-client.ts",
  "docs/openclinxr/scene-closure-2026-09-09/evidence/sc-01s.md",
  "tools/openclinxr/evidence/scene-closure/proofs/sc-01s",
  "docs/openclinxr/scene-closure-2026-09-09/evidence/sc-01s.json",
] as const;

export const SC01S_A_ROWS = ["A01"] as const;
...
export function auditScopes(
  ...
  frozen: readonly string[] = SC01S_FROZEN_SCOPES,
): string[] {
...
export function verifyReport(input: VerifyInput): VerifyResult {
  ...
  if (report.cardKey !== "SC-01S") fail(`wrong cardKey ${String(report.cardKey)}`);
  ...
  if (!isRecord(contract)) fail("missing contract section");
    ...
    for (const row of SC01S_A_ROWS) if (!aRows.includes(row)) fail(`contract.aRows omits ${row}`);
  }
  ...
  if (!isRecord(implementation)) fail("missing implementation section");
    ...
    if (!Array.isArray(changedFiles) || changedFiles.length === 0) {
      ...
      for (const entry of changedFiles) {
        ...
        const inScope = SC01S_FROZEN_SCOPES.some((scope) => {
  ...
  for (const check of checks) {
    ...
    if (!(SC01S_REQUIRED_CHECK_IDS as readonly string[]).includes(checkId)) {
  ...
  for (const required of SC01S_REQUIRED_CHECK_IDS) {
  ...
  for (const control of controls) {
    ...
    if (!(SC01S_REQUIRED_CONTROL_IDS as readonly string[]).includes(controlId)) {
  ...
  for (const required of SC01S_REQUIRED_CONTROL_IDS) {
  ...
  // DIFFERENT run than the one the card is claiming. "Bind every acknowledgment to the active
  // session/run" and "Measurements and frames must refer to the same run".
  //
  // The measurement run is derived, not declared, so a report cannot name a run that suits it: it is
  // whichever run produced the counterweight's FIXED output, because that is the run the card's
  // after-state was observed in. Every artifact a check or control leans on must be from that run.
  // EXACTLY ONE artifact is exempt: the one `counterweight.baselineOutputArtifactId` NAMES. It is
  // the before-state evidence by definition, and a control that held on BOTH revisions — the
  // strongest kind — legitimately cites it. This exemption was not in the first version of this
  // audit, which then refused this card's own `absent-scenario-id-keeps-prior-default` control for
  // citing the baseline run. The rule was too strict, not the evidence. It is narrowed to the one
  // DECLARED baseline artifact rather than to "any older artifact", so a check cannot reach a
  // second run by inventing a reason.
  ...
  if (measurementRunId !== undefined) {
    ...
    const auditRun = (kind: string, id: string, evidenceIds: readonly string[]): void => {
      for (const evidenceId of evidenceIds) {
        if (evidenceId === baselineArtifactId) continue;
