...
/**
 * SC-00's direct actual-evidence verifier.
 *
...
const EXPECTED_REPORT_PATH = `${CONTRACT_DIR}/evidence/sc-00.json`;
...
function main(): void {
  ...
  if ("error" in parsed) {
    process.stderr.write(`sc-00 verify: ${parsed.error}\n`);
    process.exitCode = 2;
    return;
  }
  if (path.normalize(parsed.report) !== path.normalize(EXPECTED_REPORT_PATH)) {
    process.stderr.write(
      `sc-00 verify: refused --report ${parsed.report}; this card may only grade ${EXPECTED_REPORT_PATH}\n`,
    );
    process.exitCode = 2;
    return;
  }
  ...
  try {
    ...
    process.stderr.write(`sc-00 verify: cannot read report — ${String(error)}\n`);
    process.exitCode = 2;
    return;
  }
  ...
  for (const document of CONTRACT_DOCUMENTS) {
    try {
      ...
      process.stderr.write(`sc-00 verify: cannot hash contract document ${document} — ${String(error)}\n`);
      process.exitCode = 2;
      return;
    }
  }
  ...
  if (result.ok) {
    process.stdout.write("sc-00 verify: every required check, control and artifact resolved.\n");
    return;
  }
  process.stderr.write(`sc-00 verify: ${result.problems.length} unmet requirement(s)\n`);
