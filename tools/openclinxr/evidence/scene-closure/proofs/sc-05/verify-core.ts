...
/** Every check the pinned SC-05 contract requires. A report cannot shrink this set. */
export const SC05_REQUIRED_CHECK_IDS = [
  "normal-encounter-selects-explicit-physician",
  "no-recorder-globals-or-direct-executor-call",
  "case-authorized-side-and-standoff-resolved",
  "swept-occupancy-includes-between-waypoint-obstacle",
  "plan-bound-to-observed-geometry-and-run",
  "arrival-error-within-0p05m",
  "settled-yaw-within-10deg",
...
export const SC05_REQUIRED_CONTROL_IDS = [
  "missing-physician-refuses",
  "nurse-in-first-clinical-slot-refuses",
  "blocked-authored-side-refuses",
  "thin-obstacle-refuses",
  "stale-plan-revision-refuses",
  "support-removed-mid-walk-stops",
  "clip-flag-without-limb-motion-fails",
  "motion-after-stop-fails",
] as const;

/** The card's frozen write roots, in the board's order. The CLI compares --scope against this. */
export const SC05_FROZEN_SCOPES = [
...
export function auditScopes(
  ...
  frozen: readonly string[] = SC05_FROZEN_SCOPES,
): string[] {
  const problems: string[] = [];
...
export function verifyReport(input: VerifyInput): VerifyResult {
  ...
  if (report.cardKey !== "SC-05") fail(`wrong cardKey ${String(report.cardKey)}`);
  ...
  if (!isRecord(contract)) fail("missing contract section");
    ...
    for (const row of SC05_A_ROWS) if (!aRows.includes(row)) fail(`contract.aRows omits ${row}`);
  }
  ...
  if (!isRecord(implementation)) fail("missing implementation section");
    ...
    if (!Array.isArray(changedFiles) || changedFiles.length === 0) {
      ...
      for (const entry of changedFiles) {
        ...
        const inScope = SC05_FROZEN_SCOPES.some((scope) => {
  ...
  for (const check of checks) {
    ...
    if (!(SC05_REQUIRED_CHECK_IDS as readonly string[]).includes(checkId)) {
  ...
  for (const control of controls) {
    ...
    if (!(SC05_REQUIRED_CONTROL_IDS as readonly string[]).includes(controlId)) {
  ...
  for (const required of SC05_REQUIRED_CONTROL_IDS) {
