...
/** Every check the pinned SC-04 contract requires. A report cannot shrink this set. */
export const SC04_REQUIRED_CHECK_IDS = [
  "selected-assets-enumerated",
  "derivation-chain-resolves",
  "adopted-shipped-public-decisions-separated",
  "first-party-terms-linked",
  "cmu-walk-terms-resolved",
  "selected-hashes-match-actual-bytes",
  "retarget-preserves-limb-integrity",
  "public-render-decision-recorded",
] as const;

/** Every negative or known-good control the pinned SC-04 contract requires. */
export const SC04_REQUIRED_CONTROL_IDS = [
  "stale-hash-refuses",
  "unknown-subcomponent-rights-refuse",
  "substituted-clip-refuses",
  "rewritten-only-provenance-refuses",
  "cleared-unchanged-asset-passes",
  "mit-repo-grants-no-blanket-clearance",
  "commercial-permission-is-not-permissive-label",
] as const;

/** The card's frozen write roots, in the board's order. The CLI compares --scope against this. */
export const SC04_FROZEN_SCOPES = [
...
export const SC04_A_ROWS = ["A06"] as const;
...
export function auditScopes(
  ...
  frozen: readonly string[] = SC04_FROZEN_SCOPES,
): string[] {
...
export function verifyReport(input: VerifyInput): VerifyResult {
  ...
  if (report.cardKey !== "SC-04") fail(`wrong cardKey ${String(report.cardKey)}`);
  ...
  if (!isRecord(contract)) fail("missing contract section");
    ...
    for (const row of SC04_A_ROWS) if (!aRows.includes(row)) fail(`contract.aRows omits ${row}`);
  }
  ...
  for (const check of checks) {
    ...
    if (!(SC04_REQUIRED_CHECK_IDS as readonly string[]).includes(checkId)) {
  ...
  for (const required of SC04_REQUIRED_CHECK_IDS) {
  ...
  for (const control of controls) {
    ...
    if (!(SC04_REQUIRED_CONTROL_IDS as readonly string[]).includes(controlId)) {
