...
function goodReport(): Record<string, unknown> {
  return {
    ...
    implementation: {
      ...
      changedFiles: ["apps/ui-xr/public/generated-humanoids/a.ts","apps/ui-xr/public/xr-assets/a.ts"],
    ...
    counterweight: {
      testIds: ["SC-04-required-behavior"],
    ...
    checks: SC04_REQUIRED_CHECK_IDS.map((checkId) => ({
      checkId,
    ...
    controls: SC04_REQUIRED_CONTROL_IDS.map((controlId) => ({
      controlId,
      trigger: "constructed",
      expected: "refusal",
    ...
    artifacts: [
      {
        ...
        objectKey: "sc-04/observations.jsonl",
...
const OBJECTS = {
  "/store/sc-04/observations.jsonl": ARTIFACT_BYTES,
  "/store/sc-04/baseline.txt": BASELINE_BYTES,
...
describe("the SC-04 evidence verifier accepts a complete control and rejects everything else", () => {
  ...
  it("(3) CHANGED artifact bytes are rejected on the hash, not merely on presence", () => {
    const result = verify(goodReport(), {
      ...OBJECTS,
      "/store/sc-04/observations.jsonl": Buffer.from("tampered\n"),
    });
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it("(5) a SYMLINK escape out of the alias root is refused even though the key looks clean", () => {
    ...
    const links = { "/store/sc-04/observations.jsonl": "/elsewhere/observations.jsonl" };
  ...
  it("(13b) a changed file outside every frozen scope is rejected, and an empty list is too", () => {
    ...
    (outside["implementation"] as Record<string, unknown>)["changedFiles"] = [
      "apps/ui-xr/public/generated-humanoids/a.ts",
  ...
  it("(14) the scope audit rejects an omitted, extra or duplicated scope", () => {
    expect(auditScopes(SC04_FROZEN_SCOPES)).toEqual([]);
    expect(auditScopes(SC04_FROZEN_SCOPES.slice(1))).toContain(`omitted --scope ${SC04_FROZEN_SCOPES[0]}`);
    expect(auditScopes([...SC04_FROZEN_SCOPES, "packages/openclinxr/telemetry"]))
      .toContain("extra --scope packages/openclinxr/telemetry");
    expect(auditScopes([...SC04_FROZEN_SCOPES, SC04_FROZEN_SCOPES[0]!]).join("\n")).toMatch(/duplicate/u);
  });
