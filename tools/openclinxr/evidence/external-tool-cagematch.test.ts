 ...
 * ## FIXED (#77)
 *
 * Measured on this machine (2026-08-06), worktree issue-77:
 *
 * Mesh2Motion
 *   - Installed: git clone Mesh2Motion/mesh2motion-app + npm install → /tmp/ocxr77_tools/mesh2motion-app
 *   - Input: mesh (GLB), not image — peds_patient_child.glb
 *   - Skeleton: 66 named joints from static/rigs/rig-human.glb (root…pelvis…fingers…ball_leaf_*)
 *   - Incumbent peds_patient_child.glb skin bones: 17; OpenClinXR canonical runtime subset: 23
 *   - Headless geometric 4-influence weights over 54_840 vertices; wall-clock ~1.3s
 *   - Decision: NOT adopted as pipeline armature (finger-heavy Mixamo-style hierarchy; Anny→23-bone
 *     owns production skin; CC0 animation library remains the residual value)
 *
 * Infinigen Indoors
 *   - Installed: indoors-stable + python3.11 venv + INFINIGEN_MINIMAL_INSTALL + git submodules
 *     (first attempt blocked: ModuleNotFoundError infinigen.infinigen_gpl.surfaces — recorded, then
 *     fixed via submodule init; Terrain re-export for empty terrain/__init__.py under minimal)
 *   - Command: generate_indoors seed0 fast_solve.gin singleroom.gin DiningRoom terrain=False
 *   - Room: 15_476_539 tris, 7_731_158 verts, 163 meshes, 189 objects
 *   - Wall-clock: 1377.98s (MAIN TOTAL 0:22:57.98)
 *   - Export: infinigen-dining-room.glb ~1.0 GB
 *   - vs Quest 100k-tris room frame: ~155×; decision NOT adopt into buildStationEnvironment
 *
 * Blocked-path honesty: install failures are written as status=blocked with blockedReason (see
 * probe functions). Both tools ran on this pass; the third contract still requires every named
 * entry to be ran|blocked with a non-empty reason when blocked.
 *
 * Remove installs: rm -rf /tmp/ocxr77_tools/mesh2motion-app /tmp/ocxr77_tools/infinigen-venv
...
describe("the candidate tools were actually run (#77)", () => {
  it("the mesh2motion probe reports a real skeleton with a bone count and named joints", async () => {
  ...
  it("the infinigen probe reports a triangle count and a generation wall-clock for a real room", async () => {
    const mod = await load();
    const run = mod["runExternalToolCagematch"] as Run | undefined;
    expect(run).toBeTypeOf("function");
    ...
    // costs to make. Neither can be written without running the tool.
  ...
  it("a tool that could not be installed is recorded as blocked with the error, not silently skipped", async () => {
