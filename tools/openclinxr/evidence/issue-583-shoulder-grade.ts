/**
 * #583 shoulder grade — lit three.js deltoid captures of the two regenerated humanoids.
 *
 * Named instrument: product three.js GLTFLoader via portless ui-xr (issue-188-footwear-grade.ts
 * precedent — the runtime loader the learner sees, not a schematic). Camera framed on the upper
 * ~35% of each figure (deltoids + shoulder line), one cell per actor.
 *
 * claimScope: appearance grade artifact for orchestrator pixel review of shoulder silhouette.
 * notEvidenceFor: clinical costume realism, drape/fabric quality, production readiness.
 */
...
const OUT_DIR = ".openclinxr/evidence/issue-583";
const OUT_PNG = path.join(OUT_DIR, "shoulder-grade.png");
const CELL_W = 480;
const CELL_H = 560;
const COLS = 2;
...
async function main(): Promise<void> {
  ...
  try {
    const page = await browser.newPage({
      viewport: { width: CELL_W * COLS, height: CELL_H + 80 },
    });
    ...
    const glbUrls = GLBS.map((p) => new URL(p.replace(/^apps\/ui-xr\/public\//, ""), server.url).toString());
    const labels = GLBS.map((p) => path.basename(p, ".glb"));
    ...
    await page.setContent(
      ...
      <h1>#583 shoulder-grade — lit three.js, deltoids / shoulder line (regenerated parent + nurse)</h1>
      <div id="sheet"></div>
      <script type="module">
      ...
      async function renderOne(url, label) {
        ...
        // Frame the shoulder line: look at the upper ~28% of body height.
        const shoulderY = box.min.y + size.y * 0.86;
        const look = new THREE.Vector3(center.x, shoulderY, center.z);
        const cam = new THREE.PerspectiveCamera(34, W / H, 0.01, 50);
        const dist = Math.max(size.x, size.z) * 2.6 + size.y * 0.35;
        cam.position.set(look.x + dist * 0.35, look.y + dist * 0.18, look.z + dist * 0.95);
        cam.lookAt(look);
    ...
    writeFileSync(
      path.join(cwd, OUT_DIR, "shoulder-grade-meta.json"),
      `${JSON.stringify({ generatedAt: new Date().toISOString(), report, out: OUT_PNG }, null, 2)}\n`,
    );
    console.log("shoulder-grade written", outAbs, "bytes", (await import("node:fs")).statSync(outAbs).size);
