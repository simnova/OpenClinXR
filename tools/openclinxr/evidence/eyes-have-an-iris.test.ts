...
describe("an eye has an iris, not a single colour", () => {
  it.fails("(1) RED: every eye material carries a base-colour texture", () => {
    requireRows();
    expect(rows.filter((r) => !r.hasTexture).map(show), "eye materials with no texture").toEqual([]);
  });

  it.fails("(2) RED COUNTERWEIGHT: the eye is not a uniform colour — a flat baseColor is refused", () => {
    requireRows();
