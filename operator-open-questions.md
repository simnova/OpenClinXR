...
- Realtime alternative evidence: `docs/openclinxr/realtime-voice-transport-spike-2026-05-04.json` proves a no-cloud bidirectional WebSocket transport harness through the Hono gateway shape and Python-compatible backend fixture, `docs/openclinxr/api-python-backend-runtime-smoke-2026-05-05.json` proves the real FastAPI backend can run locally and handle JSON/binary WebSocket frames with canonical protocol and latency fields, and `apps/ui-quest-voice-godot` now adds a source-level Quest/Godot WebSocket client contract. This proves transport/backend shape only; it does not prove real VibeVoice, Moshi, Qwen3-TTS, Grok Voice, microphone capture, Opus encode/decode, playback, or headset latency.
