...
- Current evidence: `docs/openclinxr/local-voice-runtime-benchmark-2026-05-05.json` is the latest harvested VibeVoice evidence artifact; its `generatedAt` remains `2026-05-04T15:01:12Z` because the repo-managed tool parses existing local logs/audio and does not rerun VibeVoice. The underlying local, no-cloud, no-paid VibeVoice-Realtime-0.5B file-generation run produced 3.47 seconds of audio, but took 118.9 seconds wall-clock, reported 18.17 seconds of model generation, a 5.24x real-time factor, and about 9 seconds approximate first speech-token latency. That is acceptable for slow proof-of-concept generation, but not acceptable for live station dialog.
- Realtime alternative evidence: `docs/openclinxr/realtime-voice-transport-spike-2026-05-04.json` proves a no-cloud bidirectional WebSocket transport harness through the Hono gateway shape and Python-compatible backend fixture, `docs/openclinxr/api-python-backend-runtime-smoke-2026-05-05.json` proves the real FastAPI backend can run locally and handle JSON/binary WebSocket frames with canonical protocol and latency fields, `docs/openclinxr/local-model-runtime-benchmark-2026-05-05.json` records a fresh cache-only Qwen/llama.cpp structured-output smoke, and `apps/ui-quest-voice-godot` now adds a source-level Quest/Godot WebSocket client contract. This proves transport/backend/model-adapter shape only; it does not prove real VibeVoice, Moshi, Qwen3-TTS, Grok Voice, microphone capture, Opus encode/decode, playback, or headset latency.
