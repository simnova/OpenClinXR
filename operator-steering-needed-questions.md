...
- Quest foreground performance capture: see [proposal-quest-foreground-performance-capture.md](proposal-quest-foreground-performance-capture.md). USB and ADB authorization are working, but the latest refreshed `docs/openclinxr/local-runtime-probe-2026-05-04.json` records `mWakefulness=Asleep` with `questForegroundPreflight.status=blocked`; this is a simple physical-state issue, not a new proposal blocker. The refreshed `docs/openclinxr/quest-cdp-smoke-2026-05-04.json` still classifies CDP frame sampling as hidden/inactive, so Patrick or a human operator still needs to complete the foreground headset performance template before Codex can validate frame pacing, controller latency, comfort, or in-headset text readability. Do not claim foreground Quest frame pacing from browser/CDP/MCP emulation alone.
...
- Local voice runtime proposal: Patrick approved [proposal-local-voice-runtime.md](proposals/approved/proposal-local-voice-runtime.md) on 2026-05-04 10:40:15 EDT. Codex may install and benchmark `microsoft/VibeVoice-Realtime-0.5B` in the approved local cache paths only; production use remains blocked.
- Local model benchmark proposal: Patrick approved [proposal-local-model-benchmark.md](proposals/approved/proposal-local-model-benchmark.md) on 2026-05-04 10:40:15 EDT. Codex may download and benchmark `Qwen/Qwen3-4B-GGUF` through `llama.cpp` using private env values only; production dialogue use remains blocked.
