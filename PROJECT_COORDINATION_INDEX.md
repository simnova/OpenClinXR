...
**Last slice (one-liner)**: 2026-06-03: surface active demos in review packet (pedsActiveEmotionDemo + pedsDialogueCueIdsDemo from machines in packet for peds surfaces). Guards. See AUTONOMOUS. 

**Next queued slice (do this)**: Per rebalance + surface active in review packet: continue local gen. Next: materialization continuation or full active in more runtime builders/review for peds. Record Per-Slice, focused verif, post+guards, pick. (Or pivot per worker if higher value.)
