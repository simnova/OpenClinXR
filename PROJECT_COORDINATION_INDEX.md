...
**Next queued slice (do this)**: wire produced url continuation or richer authoring or full persistence (product): per rehydrate (IWSDK spike sidecar + multimodal note addressed) + guardrail; after loco/gaze/lip wire (this slice), highest wire produced url (use in main gltf load or consumer attach for more materialization/review surfaces) or richer env authoring in factory or full mongo save call. (IWSDK: spike sidecar only per AGENTS + user note; installed in spike this turn for evidence; no main add. Multimodal: read_file png for WebXR desc to verif loco/gaze/lip + deeper in canvas.) Record Per-Slice for loco/gaze/lip wire, focused, exact post+guards, release, rehydrate, pick, continue. (Continue local det.)
**Last slice (one-liner)**: 2026-06-03 loco/gaze/lip live apply from gen drive to humanoid in player (product): live wire in updateGeneratedHumanoidAnimations (drive param + position/rotation/morph apply for loco/gaze/lip from gen drive); call passes genDrive (from pedsRuntimeDrive/floor/global); IWSDK spike sidecar + multimodal note. Blueprint tie Q1/2/4/5. Touched: main.ts + spike + states. Evidence: lease; biome pre warnings; spike install; guards. See AUTONOMOUS. Next from fresh: wire produced or richer or persistence; record, focused, post+guards, release, pick. (Continue.)
