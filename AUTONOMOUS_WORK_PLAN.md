...
2026-05-27 dependency/currentness hardening slice: Product path advanced: repo-native asset/tooling gates for the case-definition-driven encounter factory. Blueprint/factory tie: GLTF source-smoke readiness can now consume `@gltf-transform/core` Node API evidence instead of requiring the legacy `gltf-pipeline` report shape, while IWSDK remains sidecar-gated because latest `0.4.1` Vite plugins still peer on Vite 7. Touched files: IWSDK currentness checker/evidence, asset-production readiness benchmark, local runtime probe, dependency hygiene docs, GLTF Transform decision record, generated doc/artifact registries. Evidence: `docs/openclinxr/iwsdk-npm-currentness-2026-05-27.json`, `docs/openclinxr/iwsdk-npm-metadata-snapshot-2026-05-27.json`, `docs/openclinxr/gltf-transform-smoke-2026-05-27.json`, `docs/openclinxr/asset-production-readiness-benchmark-2026-05-27.json`. Next queued slice: update security/license notes and asset-pipeline docs, then re-run e18e before deciding whether `gltf-pipeline` can be removed or isolated.
