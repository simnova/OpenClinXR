...
2. `PROJECT_STATUS.md` is the canonical state file (autonomy, priority, active work, backlog, strategy, per-slice checkpoints).
3. `docs/openclinxr/worker-backlog-and-validation-matrix.md` is the worker ownership and validation map.
4. `operator-steering-needed-questions.md` contains true blockers and hardware/operator instructions.
5. `operator-open-questions.md` contains nonblocking steering questions and recommended defaults.
6. `docs/agent-factory/**`, `agents/**`, and `iterations/**` are persistent multi-agent memory and governance evidence. Use them to realign task selection, not to restart broad planning loops unless the active plan explicitly calls for one.
7. `PROJECT_COORDINATION_INDEX.md`, `AUTONOMOUS_WORK_PLAN.md` are historical audit ledgers only (see PROJECT_STATUS.md "Historical Audit Ledgers" section).
