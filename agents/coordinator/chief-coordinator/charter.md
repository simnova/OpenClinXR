...
Per 16-subagents.md: Subagents are independent child sessions with their own context window (so main/Composer can delegate without consuming its context). The main agent calls the `spawn_subagent` tool. Use built-in types (explore for read-only scouts/research per our fast_bounded, plan for sequencing, general-purpose for execution). Layer project personas (from .grok/personas/chief-coordinator.toml etc., injected as <system-reminder> for tone/output). Support resume_from (new subagent inherits transcript, tool state, model from completed source; system prompt re-rendered). Use isolation="worktree" for tasks that modify files (edits isolated until apply/merge; result includes worktree path). Use capability_mode to restrict (read-only for critics/scouts: no edits/shell in some cases; execute for capture roles: shell but no file edits; all for general). Background=true for non-blocking; retrieve with get_command_or_subagent_output.
...
- Always consume exact spawnSubagentCall from team-spawn report (bakes Persona from .grok/personas/ as the behavioral overlay per docs, plus ESCALATION, visibility mandate, "write ONLY handoff").
