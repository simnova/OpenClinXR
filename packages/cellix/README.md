...
**Upstream pin:** `CellixJs/cellixjs` `adf3bc9deb` (2026-08-27). Local clone: `/Volumes/files/src/cellixjs` on `main`. `provider-contracts` and `trace-ledger` are OpenClinXR seedwork extractions, not upstream packages. `server-mongodb-memory-mock` is a local MongoClient helper; upstream is `@cellix/server-mongodb-memory-mock-seedwork` (mongoose replica set).
