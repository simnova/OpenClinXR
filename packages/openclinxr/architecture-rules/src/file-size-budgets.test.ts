...
const SIZE_FREEZE: Record<string, { maxLines: number; reason: string }> = {
  ...
  "apps/api/src/app.ts": { maxLines: 2412, reason: "API composition root mid-migration to fluent DI: types->api-types.ts, helpers->api-support.ts done; NEXT carve 57 inline routes into per-domain createXRoutes(ctx) modules, then introduce ApiApplication builder + compose.ts (Atlantis composition-root pattern)" },
