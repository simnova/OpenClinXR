  ...
  it("routes and generates actor responses through promoted session-state context", async () => {
    ...
    expect(packet.timeline.map((entry) => entry.summary)).toEqual(expect.arrayContaining([
      ...
      expect.stringMatching(
        /^nurse_maria_alvarez_v1 response generated by capture-model \(spoken_actor_response\); guardrail pass; durable event durable:\/\/station-runs\/run_ed_chest_pain_priority_v1_learner_001\/events\/7; ActorTurnPlan /,
      ),
    ]));
