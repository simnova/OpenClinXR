...
import { BlueprintCoveragePanel } from "./blueprint-coverage-panel.js";
...
describe("BlueprintCoveragePanel", () => {
  it("renders every construct gap and persists a fail-closed activation attempt", async () => {
    ...
    expect(within(panel).getByLabelText("Assembled exam form coverage")).toHaveTextContent(
      "Assembled form blueprint_incomplete",
    );
  ...
  it("resets staged reviews and saved success when same-version plan content changes", async () => {
    ...
    const { rerender } = render(
      <BlueprintCoveragePanel input={workflowInput(true)} {...props} />,
    );
    const panel = screen.getByLabelText("Blueprint sampling-plan activation");
    fireEvent.click(
    ...
    refreshed.blueprintRevision.requirements = refreshed.blueprintRevision.requirements.map(
      (coverageRequirement) =>
        coverageRequirement.dimension === "reasoning"
          ? { ...coverageRequirement, value: "refreshed_reasoning" }
          : coverageRequirement,
    );
    rerender(<BlueprintCoveragePanel input={refreshed} {...props} />);
