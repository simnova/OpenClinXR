...
export function BlueprintCoveragePanel(props: BlueprintCoverageWorkflowProps): ReactElement {
...
function VersionPinnedBlueprintCoverageWorkflow({
  ...
  const assembled = useMemo(
    () => assemblePlanForm(input, substitutions),
    [input, substitutions],
  );
  ...
  return (
    <section className="workbench-panel" aria-label="Blueprint sampling-plan activation">
      ...
      <Typography.Paragraph type="secondary" aria-label="Assembled exam form coverage">
        {assembled.status === "assembled"
          ? `Assembled form ${assembled.form.status}; ${assembled.form.assemblyIssues.length} assembly issues.`
          : `Assembly refused: ${assembled.message}`}
      </Typography.Paragraph>
