 ...
 * by its persistence port. Occupied-break close uses the existing run function
 * so tests never need a new public export.
 */
export async function completeExamFormRunWithAttemptManifest(
  ...
  const runReadyForTerminalStation = closeOccupiedBreakThroughExistingRunApi(input);
  assertTerminalCompletionInput(runReadyForTerminalStation, input);

  const completedRun = advanceExamFormRunStation(
    runReadyForTerminalStation,
    input.finalStationCompletion,
  );
...
function closeOccupiedBreakThroughExistingRunApi(
  input: CompleteExamFormRunWithAttemptManifestInput,
): ExamFormRunState {
  if (input.run.currentPhase.kind !== "break") {
    return input.run;
  }

  const afterStationOrder = input.run.currentPhase.afterStationOrder;
  const breakEvidence = input.breakEvidence.find(
    (entry) => entry.afterStationOrder === afterStationOrder,
  );
  if (!breakEvidence) {
    throw new Error("attempt manifest requires evidence for every timed break");
  }

  const afterBreak = advanceExamFormRunBreak(input.run, {
    endedAtFormSecond: breakEvidence.ended.formAtSecond,
    recordedAtIso: breakEvidence.ended.recordedAtIso,
  });
  if (afterBreak.currentPhase.kind === "break") {
    throw new Error("attempt manifest completion cannot bypass an occupied break");
  }
  return afterBreak;
}

function assertTerminalCompletionInput(
  ...
  if (run.currentPhase.kind !== "station") {
    throw new Error("attempt manifest completion cannot bypass an occupied break");
  }
  const station = currentExamFormRunStation(run);
  const lastStation = run.queue.stationQueue.at(-1);
...
function outcomeFor(
  ...
  const matches = outcomes.filter((recorded) => recorded.stationOrder === station.stationOrder);
  if (matches.length !== 1) {
    throw new Error(
