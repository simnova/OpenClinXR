...
import type { ExamForm } from "../types.js";
import {
...
export function buildSamplingPlan(input: BuildSamplingPlanInput): SamplingPlan {
  ...
  const assembledForm = resolveAssembledForm(input, blockers);
  if (assembledForm) {
    pinAssembledForm(assembledForm, coverageMatrix, blockers);
  }
...
function resolveAssembledForm(
  input: BuildSamplingPlanInput,
  blockers: string[],
): ExamForm | undefined {
  if (input.assembledForm) {
    if (input.assembledForm.examFormId !== input.examFormId) {
      blockers.push("assembly_exam_form_id_mismatch");
    }
    if (input.assembledForm.blueprintId !== input.blueprintRevision.blueprint.blueprintId) {
      blockers.push("assembly_blueprint_id_mismatch");
    }
    return input.assembledForm;
  }
  if (!input.stationScenarios) {
    return undefined;
  }
  try {
    return assembleExamForm({
      examFormId: input.examFormId,
      blueprint: input.blueprintRevision.blueprint,
      scenarios: [...input.stationScenarios],
    });
  } catch (error) {
    blockers.push(
      error instanceof Error ? `assembly_refused:${error.message}` : "assembly_refused",
    );
    return undefined;
  }
}

function pinAssembledForm(
  form: ExamForm,
  coverageMatrix: readonly SamplingPlanCoverageMatrixRow[],
  blockers: string[],
): void {
  for (const issue of form.assemblyIssues) {
    blockers.push(`assembly:${issue}`);
  }
  if (form.status !== "ready_for_review") {
    blockers.push(`assembly_status:${form.status}`);
  }
  const pins = coverageMatrix.map((row) => `${row.scenarioId}@${row.scenarioVersion}`);
  const refs = form.stationRefs.map((ref) => `${ref.scenarioId}@${ref.scenarioVersion}`);
  if (JSON.stringify(pins) !== JSON.stringify(refs)) {
    blockers.push("assembly_station_revision_mismatch");
  }
}
