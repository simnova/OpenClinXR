...
import type { ExamBlueprint, ExamForm } from "../types.js";
...
export type BuildSamplingPlanInput = {
  ...
  /**
   * Optional station bodies for `assembleExamForm`. When present, activation also
   * fails closed on assembly coverage/status and station-revision mismatch.
   */
  stationScenarios?: readonly Scenario[];
  /** Optional already-assembled form; must match `examFormId` / blueprint / station pins. */
  assembledForm?: ExamForm;
};
