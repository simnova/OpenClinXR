...
import {
  ...
  facultyCaptionForPlan,
  facultyDroppedTagLog,
  type FacultyActorTurnReplay,
  prosodyNeutralizedFromPayload,
  summarizeActorTurnExecution,
  summarizeActorTurnPlan,
...
function summarizeTimelineEvent(
  ...
  if (event.eventType === "actor.response.generated" && event.actorId) {
    ...
    const authored = authoredTurnReplayFromPayload(event.payload);
    ...
    return [
      ...
      authored ? summarizeAuthoredTurnReplay(authored) : undefined,
      plan
        ? summarizeActorTurnPlan(plan, facultyCaptionForPlan(plan), facultyDroppedTagLog(plan, undefined))
        : undefined,
    ].filter(Boolean).join("; ");
  }
  ...
  if (event.eventType === "voice.audio.generated" && event.actorId) {
    ...
    return [
      `${event.actorId} voice audio generated`,
      durableEventSummary(event),
      execution
        ? summarizeActorTurnExecution(execution, prosodyNeutralizedFromPayload(event.payload))
        : undefined,
    ].filter(Boolean).join("; ");
  }

  if (event.eventType === "actor.turn.planned") {
    const plan = actorTurnPlanFromPayload(event.payload);
    if (plan) {
      return summarizeActorTurnPlan(plan, facultyCaptionForPlan(plan), facultyDroppedTagLog(plan, undefined));
    }
  }

  if (event.eventType === "actor.turn.executed") {
    const execution = actorTurnExecutionFromPayload(event.payload);
    if (execution) {
      return summarizeActorTurnExecution(execution, prosodyNeutralizedFromPayload(event.payload));
    }
  }
