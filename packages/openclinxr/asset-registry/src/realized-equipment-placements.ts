...
/**
 * The collapse report for a built bundle's equipment list. Lives here beside the realized
 * placement identity it reports on, and out of runtime-bundles.ts, which is at a shrink-only
 * frozen ceiling asking for builder / shape to separate.
 */
export function buildEquipmentPlacementReport(
  equipment: readonly EncounterRuntimeEquipmentAsset[],
): EquipmentPlacementReport {
  ...
  // A row is reported ONLY for an asset id this bundle actually contains. An earlier draft
  // appended a hardcoded iv_stand_equipment row whenever the bundle held more than one item,
  // because the planted clause asserted that id against a fixture built entirely from
  // ecg_cart_equipment. A report that names assets the bundle does not hold is worse than no
  // report: it is the collapse-detection surface itself lying about what collapsed.
  return { collapsed };
}
