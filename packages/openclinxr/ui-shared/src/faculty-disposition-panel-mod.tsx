...
import { asRefusal, asTrail } from "./faculty-disposition-codec-mod.js";
import type {
...
} from "./faculty-disposition-record-mod.js";
import {
  DISPOSITION_LABEL,
  ...
  useEffect(() => {
    ...
    loadTrail(nextExamRunId)
      .then((loadedTrail) => {
        ...
        setState({ status: "ready", examRunId: nextExamRunId, trail: loadedTrail });
        setReviewerId(loadedTrail.current?.reviewerId ?? loadedTrail.decisions[0]?.reviewerId ?? "");
      })
      .catch((error: unknown) => {
        if (active) {
          setState({
