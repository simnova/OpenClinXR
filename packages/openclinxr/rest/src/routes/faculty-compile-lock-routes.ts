...
/** Workspace-root default for direct callers; the composition root passes its own. */
function defaultLockRepoRoot(): string {
  return resolveCompileLocksRepoRoot();
}
 ...
 */
export function registerFacultyCompileLockRoutes(
  app: Hono<{ Variables: ApiAppVariables }>,
  _ctx: ApiAppContext,
  repoRoot: () => string = defaultLockRepoRoot,
): void {
