...
export function compileLocksPathFor(scenarioId: string, repoRoot: string = defaultRepoRoot()): string {
...
/** Workspace root default: walk up from cwd to pnpm-workspace.yaml. */
...
/** Resolve the workspace root from an explicit directory (composition root passes its own). */
export function resolveCompileLocksRepoRoot(fromDir: string = process.cwd()): string {
  return fromDir;
}

/** Read the current lock record for a scenario; a missing file yields an empty record. */
export async function readFacultyCompileLocksRecord(scenarioId: string, repoRoot?: string): Promise<FacultyCompileLockFile> {
  const filePath = compileLocksPathFor(scenarioId, repoRoot);
  let raw: string;
  try {
...
export async function writeFacultyCompileLock(
  ...
  const existing = await readFacultyCompileLocksRecord(scenarioId, repoRoot);
