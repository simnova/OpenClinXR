/**
 * Clinical device builders: wall clock, monitors, exam table, cuff,
 * exam-zone dressing, generic fallback (#140).
 *
 * Split from station-equipment-builders.ts so the dispatcher parent stays
 * under the packages/ 500-line zone budget. Same builders, new address.
 */
...
import type { EquipmentFamily } from "./station-equipment-families.js";
...
export type ChairVariant = "clinic" | "parent" | "safe_room";

/** Chair family — shared seat/back class, distinct footprints per role. */
export function buildSimpleChairEquipment(equipmentId: string, variant: ChairVariant = "clinic"): Group {
  const root = new Group();
  root.name = `openclinxr.equipment.${equipmentId}`;
  if (variant === "parent") {
...
export type HandheldKind = "pulse_ox" | "nebulizer" | "inhaler" | "o2_port";

/** Compact device on a stand — distinct body geometry per kind (not color-only). */
export function buildHandheldDeviceOnStand(equipmentId: string, kind: HandheldKind): Group {
  const root = new Group();
  root.name = `openclinxr.equipment.${equipmentId}`;
