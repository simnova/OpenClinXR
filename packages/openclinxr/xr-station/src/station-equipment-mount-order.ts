...
/** True when the fixture owns this asset id's role class (skip dual-mount). */
export function stationEquipmentSuppressedByFixture(
  equipmentId: string,
  owned: Set<string>,
): boolean {
  return equipmentSuppressedByFixtureOwnership(
    equipmentId,
    owned as Set<Parameters<typeof equipmentSuppressedByFixtureOwnership>[1]>,
  );
}
