...
// ---------------------------------------------------------------------------
// JoltCandidateAdapter — basic behaviour
// ---------------------------------------------------------------------------
describe("JoltCandidateAdapter", () => {
  it("has engineId 'jolt-candidate' in meta", () => {
    const adapter = new JoltCandidateAdapter();
    expect(adapter.meta.engineId).toBe("jolt-candidate");
  ...
  it("step creates contact with Jolt-style material properties", () => {
    const adapter = new JoltCandidateAdapter(42);
    adapter.step({
      tick: 0,
      handedness: "right",
    ...
    // Jolt stores material properties on contacts
    expect(contact.combinedRestitution).toBeGreaterThanOrEqual(0);
    expect(contact.combinedFriction).toBeGreaterThanOrEqual(0);
    // Jolt stores speculative offset
    expect(contact.speculativeOffset).toBeGreaterThan(0);
  });
  ...
  it("applySnapshot restores full state including contacts and broad-phase", () => {
    const adapter = new JoltCandidateAdapter(42);
    ...
    // Jolt broad-phase should be restored
    expect(stateRestored.broadPhaseCells).toBeDefined();
  });

  it("sub-steps are visible in state (subStep field toggles 0→1 within a tick)", () => {
    const adapter = new JoltCandidateAdapter(42);
...
describe("jolt C6 replay equivalence", () => {
  ...
  it("different seeds → different checksums, but same seed → same checksums", () => {
    ...
    const adapter42 = new JoltCandidateAdapter(42);
    const trace42 = replayInputLog(adapter42, log, 30);

    const adapter99 = new JoltCandidateAdapter(99);
    ...
    const adapter42b = new JoltCandidateAdapter(42);
...
// ---------------------------------------------------------------------------
describe("jolt C6 replay with palpation log", () => {
