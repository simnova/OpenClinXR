...
export function validateModelVettingReport(value) {
    const errors = [];
    if (!isRecord(value))
    ...
    if (!Array.isArray(candidates) || candidates.length === 0)
        errors.push("/candidates must be nonempty array");
...
function modelCandidateFromAnnyCandidate(candidate) {
...
function labModesForCandidate(candidate, captureArtifacts) {
    ...
    const morphVideoPaths = [captureArtifacts.morphVisemeTimelineCapture, captureArtifacts.emotionTransitionCapture].filter(Boolean);
    return [
        {
        ...
        },
        {
            ...
            blockers: blockersIf(!(candidate.glb.animationCount > 0), "animation_clips_missing", !(candidate.glb.clinicalIdlePoseClipCount > 0), "clinical_idle_or_conversation_clip_missing", !candidate.rigControlEvidence.locomotionPostureClipPresent, "locomotion_posture_clip_missing", !turntableVideoPresent, "turntable_animation_video_missing"),
        },
        {
            ...
            requiredArtifactKinds: ["json", "screenshot"],
            capturedArtifactPaths: [...captureArtifacts.fixedCameraScreenshots],
            blockers: blockersIf(!(candidate.glb.materialCount > 0), "materials_missing", !candidate.provenance.sourceOriginChainPresent, "source_origin_chain_missing", !candidate.provenance.licenseChainPresent, "license_chain_missing", !screenshotsPresent, "material_fixed_camera_screenshots_missing"),
        },
        {
            ...
            requiredArtifactKinds: ["json"],
            capturedArtifactPaths: [],
            blockers: blockersIf(!(candidate.glb.byteLength > 0), "glb_empty", !(candidate.glb.byteLength < 25_000_000), "glb_byte_budget_exceeded_for_lab_gate", !(candidate.glb.vertexCount < 150_000), "vertex_budget_exceeded_for_lab_gate"),
        },
    ];
}
function validateCandidate(value, path, errors) {
    ...
    if (isRecord(falseGateValue)) {
        if (typeof falseGateValue["realAnny"] !== "boolean")
            errors.push(`${path}/falseGates/realAnny must be boolean`);
    ...
    if (!Array.isArray(labModes) || labModes.length !== requiredModeIds.length)
        errors.push(`${path}/labModes must include exactly the six isolated lab modes`);
    if (Array.isArray(labModes)) {
        ...
        for (const modeId of requiredModeIds) {
            if (!modeIds.includes(modeId))
                errors.push(`${path}/labModes must include ${modeId}`);
        }
        if (new Set(modeIds).size !== modeIds.length)
            errors.push(`${path}/labModes must not contain duplicate mode ids`);
        for (const [index, mode] of labModes.entries()) {
            if (!isRecord(mode)) {
                errors.push(`${path}/labModes/${index} must be object`);
                continue;
            }
            if (!["pass", "warn", "block"].includes(String(mode["status"])))
                errors.push(`${path}/labModes/${index}/status must be pass, warn, or block`);
            if (!Array.isArray(mode["evidence"]))
                errors.push(`${path}/labModes/${index}/evidence must be array`);
            if (!Array.isArray(mode["requiredArtifactKinds"]))
                errors.push(`${path}/labModes/${index}/requiredArtifactKinds must be array`);
            if (!Array.isArray(mode["capturedArtifactPaths"]))
                errors.push(`${path}/labModes/${index}/capturedArtifactPaths must be array`);
            if (!Array.isArray(mode["blockers"]))
                errors.push(`${path}/labModes/${index}/blockers must be array`);
        }
    }
    const structuralMetrics = value["structuralMetrics"];
    requireRecord(structuralMetrics, `${path}/structuralMetrics`, errors);
    if (isRecord(structuralMetrics))
        requireSha(structuralMetrics["sha256"], `${path}/structuralMetrics/sha256`, errors);
    ...
    if (isRecord(provenance)) {
        ...
        if (!Array.isArray(provenance["auditPointers"]) || provenance["auditPointers"].length === 0)
            errors.push(`${path}/provenance/auditPointers must be nonempty array`);
    ...
    if (isRecord(captureArtifacts) && !Array.isArray(captureArtifacts["fixedCameraScreenshots"]))
        errors.push(`${path}/captureArtifacts/fixedCameraScreenshots must be array`);
}
function validateProviderBoundary(value, path, errors) {
    requireRecord(value, path, errors);
    if (!isRecord(value))
        return;
...
function localMetadataOnlyProviderBoundary() {
    return {
...
function falseGates(usesRealAnnyForwardPass = false) {
    return {
...
function blockersIf(...pairs) {
    const blockers = [];
    for (let index = 0; index < pairs.length; index += 2) {
        if (pairs[index] === true)
            blockers.push(String(pairs[index + 1]));
    }
    return blockers;
}
...
function requireString(value, path, errors) {
    if (typeof value !== "string" || value.length === 0)
        errors.push(`${path} must be nonempty string`);
}
function requireSha(value, path, errors) {
    if (typeof value !== "string" || !/^[a-f0-9]{64}$/u.test(value))
        errors.push(`${path} must be sha256 hex`);
}
function requireLiteral(value, expected, path, errors) {
    if (value !== expected)
        errors.push(`${path} must be ${String(expected)}`);
}
function requireStringArrayIncludes(value, expected, path, errors) {
...
function isolatedCaptureComplete(captureArtifacts) {
