export const PEDS_ASTHMA_PATIENT_VISeme_DIALOGUE_UTTERANCE = "Maya Johnson: It is hard to breathe and my chest feels tight.";
...
export function phonemeSequenceForDialogue(text) {
    const spoken = text.replace(/^[^:]+:\s*/u, "").toLowerCase();
    const sequence = [];
    for (const char of spoken) {
        if (/[aeiou]/u.test(char))
            sequence.push(char);
        else if (/[bmp]/u.test(char))
            sequence.push("m");
        else if (/[fv]/u.test(char))
            sequence.push("f");
        else if (/[tdnlsz]/u.test(char))
            sequence.push("t");
        else if (/[kgqcr]/u.test(char))
            sequence.push("k");
        else if (/[wy]/u.test(char))
            sequence.push("w");
        else if (/[.!?]/u.test(char))
            sequence.push("sil");
    }
...
export function visemeForPhoneme(phoneme) {
    if (phoneme === "sil")
        return "rest";
    if (phoneme === "m")
        return "closed";
    ...
    if (phoneme === "w")
        return "rounded";
    if (phoneme === "a" || phoneme === "o")
        return "open";
    if (phoneme === "e" || phoneme === "i")
        return "wide";
    return "mid";
}
export function visemeOpenness(viseme) {
    return VISEME_OPENNESS[viseme] ?? 0.35;
}
export function humanoidDialogueDurationMs(phonemeCount, extendedCapture = false) {
...
export function buildVisemeTimelineFromDialogue(dialogueText, input) {
...
export function visemeAtTimelineProgress(timeline, progress) {
    const clampedProgress = Math.min(1, Math.max(0, progress));
    const index = Math.min(timeline.visemeSequence.length - 1, Math.max(0, Math.floor(clampedProgress * timeline.visemeSequence.length)));
...
export function applyMorphTargetVisemeCue(root, openness, viseme) {
