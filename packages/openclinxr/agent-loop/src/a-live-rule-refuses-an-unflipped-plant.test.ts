 ...
 *   the-shipped-room-matches-its-declared-shape.test.ts   1 expected fail  (#227, unlanded)
 ...
 *
 * Detail strings: fail names the FILE and the REMAINING COUNT ("still has 1 unflipped it.fails
...
describe("a live: rule refuses an unflipped plant", () => {
  ...
  it("(2) live: FAILS on an unflipped plant, and says WHY", async () => {
    ...
    expect(check.passed, "1 it.fails clause remains — the current-main case where run: exits 0").toBe(false);
    expect(
      String(check.detail),
      "must fail because REDs remain, not because the rule is unrecognised",
    ).not.toMatch(/unsupported rule/iu);
    expect(String(check.detail), "the detail names how many REDs are still unflipped").toMatch(/\b1\b/u);
  });
