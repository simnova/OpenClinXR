...
def _repo_root() -> Path:
    ...
    for p in [here.parent, *here.parents]:
        if (p / "pnpm-workspace.yaml").is_file():
            return p
    raise RuntimeError("workspace root (pnpm-workspace.yaml) not found from body_param_stage.py")


# Sibling MakeClothes helpers stay under tools/; this stage script lives in the station package.
_TOOLS_OPENCLINXR = _repo_root() / "tools" / "openclinxr"
_MAKECLOTHES_DIR = str(_TOOLS_OPENCLINXR / "asset-pipeline" / "makeclothes")
...
def _load_hm08_rig_stage():
    ...
    rig_path = _TOOLS_OPENCLINXR / "evidence" / "blender" / "hm08_rig_carry_stage.py"
...
def _ensure_probe_machinery_path() -> None:
    ...
    """
    probe_dir = _TOOLS_OPENCLINXR / "evidence" / "blender"
