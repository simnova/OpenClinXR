...
/**
 * PLACEHOLDER for `clutch_body_region`, owned by M4 (tsk_eed004e50d19be54).
 *
 * This module is the OWNERSHIP SLOT this card (tsk_51ffcc3e1a8fdea8) established: the registry
 * resolves `clutch_body_region` to this module's `compile` and M4 replaces THIS BODY only — never
 * `primitive-registry.ts`. The placeholder returns a legal EMPTY-tracks fragment so the seam is
 * canonical end to end while the clutch solver lives with its owner.
 *
