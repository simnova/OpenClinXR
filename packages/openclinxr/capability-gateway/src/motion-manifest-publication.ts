...
export function publishRealTouchMotion(input: RealTouchPublicationInput): RealTouchPublicationOutput {
  ...
  const glbBytes = bakeMotionProgramToGlb({
    clipId: clip.clipId,
    compileIdentity: clip.compileIdentity,
    tracks: clip.tracks,
  });
  const readbackClipId = readMotionGlbClipId(glbBytes);
  if (readbackClipId !== clip.clipId) {
...
/**
 * Bind-world anchor proxies for the promoted MPFB rig: landmark positions plus
 * region direction offsets, scaled by torso height and the whole bind frame's
 * maximum absolute lateral/depth coordinates. These whole-rig bounds differ from
 * the canonical anchor producer's shoulder half-width; this does not establish
 * canonical anchor parity, anatomical contact, or clinical validity.
 * The offsets are retained locally because the canonical anchor producer is not
 * part of the supported public entrypoint; the compilation proof is interchange
 * publication on the measured rig, not a measurement of body-surface contact.
 */
