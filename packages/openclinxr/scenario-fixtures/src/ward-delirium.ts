...
export const wardDeliriumScenario = draftScenario({
  ...
  actors: [
    ...
    ),
    {
      ...
      // so the bake reused the nurse's Anny reference and a learner met the nurse
      // and the senior resident as the same woman in a lab coat. The case now
      // authors the person: a male senior resident, mid-30s, 172 cm — a
      // distinct body from the co-present female-presenting nurse-class body.
      // The sex is authored for the PERSON (case definition), not derived from
      // the role descriptor (#664: the lookup must never invent a person's sex).
      ...actor(
      ...
      phenotype: {
        ...
        clothing_color: "clinical_white",
        role_visual_cue: "senior_resident_physician",
        wardrobeRole: "physician_clinical",
        garmentLayers: ["scrub_shirt", "scrub_pants", "lab_coat"],
        fabricPalette: "clinical_teal_and_white",
        materialFinish: "cotton_matte",
        accessoryMarkers: ["stethoscope", "badge"],
        fitProfile: "adult_clinical_average_fit",
      },
    },
  ],
  requiredTraceTags: [
