...
export const COMPOSITION_ROOT_APP_BUDGETS: readonly CompositionRootAppBudget[] = [
  ...
  },
  {
    ...
    maxLines: 6026,
    reason:
      "6083 -> 6045 by extractions, then 6055 when the boot path gained role-based cast binding on 2026-09-09 when the supported-actor placement composition moved to @openclinxr/xr-runtime-state; 6069 -> 6039 on 2026-09-10 when SC-06 moved the four bundle-inspection helpers (inspectPinnedBundleIdentity, inspectBundleEligibility and their two private helpers, 53 lines of pure inspection over an asset-registry type) out of encounter-bundle-boot into @openclinxr/asset-registry/encounter-bundle-admission, and spent part of the freed lines wiring the frozen-scene reopen into the boot path and the frame loop; 6039 -> 6048 on 2026-09-10 when SC-06 round 4 made the frame-loop admission load-bearing (the live bedside approach is stepped only once the frozen plan is admitted and reproduced) and passed the selected scenario's own station id to the bundle producer through the case's frozen-plan binding; the field was then committed as 6037, which never matched measureAppSource on that same tree. Confirmed 2026-09-26 by archiving apps/ui-xr/src at 633b90faf, 4ee4b38d6 and 96c0c2072 and counting logical lines directly outside the test runner: all three measure 6026, not 6037/6048. Pre-existing since the commit that introduced the 6037 figure; unrelated to any change on this branch. Corrected to the true measured ceiling, 6026 (a shrink, per ratchet semantics); ratchet toward the CellixJs largest app (ui-community, 19 files / 1,506 lines) — behaviour moves to packages",
  },
] as const;
