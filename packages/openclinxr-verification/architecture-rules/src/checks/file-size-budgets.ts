...
export const SIZE_FREEZE: Record<string, { maxLines: number; reason: string }> = {
  "packages/openclinxr/xr-locomotion/src/locomotion.ts": { maxLines: 733, reason: "keyboard/XR/hand-gesture locomotion and hand select, extracted whole from main.ts — split by input source next" },
  "packages/openclinxr/xr-trace-readiness/src/trace-panels.ts": { maxLines: 569, reason: "trace, readiness and manual-evidence panels, extracted whole from main.ts — split panel-by-panel next" },
  "packages/openclinxr/xr-asset-loading/src/generated-loaders.ts": { maxLines: 574, reason: "generated humanoid/equipment/environment slot loaders, extracted whole from main.ts — split by asset kind next" },
  "packages/openclinxr/xr-asset-loading/src/role-visuals.ts": { maxLines: 536, reason: "role-specific humanoid visuals and identity cues, extracted whole from main.ts — split by role next" },
  "packages/openclinxr/xr-scene/src/humanoid-runtime-asset-url.ts": { maxLines: 502, reason: "cast path resolution; crossed the 500 budget when the ED alias comment landed — split the per-scenario maps out next" },
  "apps/ui-xr/src/main.ts": { maxLines: 4837, reason: "XR runtime god-file — #1 paydown; split by subsystem (scene, input, locomotion, capture, HUD); #57 exam-form boot extract; #44 station-environment extract; #72 actor-floor-composition + encounter-actor-framing extract; #83 physics-touch apply extract + seated posture; #115 station-context + station-vitals extract (honest unauthored vitals); capture-evidence extract (capture-mode predicates, scene-manifest/use-gate/slot/xr-entry/equipment-mount evidence builders, panel + visual-review predicates); #800 exam-run wiring extract (query params, sequence stepping, station outcomes as arguments); humanoid-animation extract (generated-humanoid frame loop, speech cue, emotion expression, gaze cue, virtual-device pulses, face/morph rig controls, gaze evidence builders); xr-locomotion extract (keyboard/xr/hand-gesture locomotion, hand select, portal transition, examinee trail, role/scenario posture); xr-scene-cues extract (clinical/text panels, nameplates, virtual-device affordance, room props + detail visuals, humanoid mouth/gaze/focus/expression/detail/collision cues, trace + environment visuals, naming/role/pediatric evidence, actor framing); xr-asset-loading extract (generated humanoid/equipment/environment loaders, variant paths, role + identity visuals, set dressing, clip names, material tint); xr-station-room extract (station room shell + environment loading); xr-actor-dialogue extract (actor dialogue store: initial text, trace-tag turns, peds playback, adaptive branch, humanoid speech, emotion context, gaze targets); xr-station-room actor-staging extract (four station actor-slot mounts + nameplate/scene-object-name helpers); xr-exam-flow extract (exam-flow store: phase-trace intents, station outcomes, form/form-run/summary evidence builders, timer advances)" },
  ...
  "packages/openclinxr/rest/src/api-types.ts": { maxLines: 682, reason: "REST DTO type surface; moved from apps/api in the composition-root migration — split by domain area" },
  ...
  "packages/openclinxr/ui-route-shared/src/admin-api-client-types.ts": { maxLines: 1362, reason: "admin DTO type surface (moved from apps/ui-admin in the composition-root migration) — split by domain area next" },
  "packages/openclinxr/ui-route-shared/src/admin-api-client.ts": { maxLines: 599, reason: "admin fetch client (moved from apps/ui-admin in the composition-root migration) — split by domain area next" },
  ...
  "packages/openclinxr/ui-route-admin/src/runtime-selection-review-packet-panel.tsx": { maxLines: 682, reason: "large panel — extract sub-sections/containers (moved from apps/ui-admin in the composition-root migration)" },
  "packages/openclinxr/ui-route-admin/src/case-authoring-workbench.tsx": { maxLines: 672, reason: "large authoring panel — extract form sections (pure data helpers moved to case-authoring-io.ts) (moved from apps/ui-admin in the composition-root migration)" },
  ...
  "packages/openclinxr/ui-route-admin/src/environment-generation-queue-panel.tsx": { maxLines: 575, reason: "large panel — extract sub-sections/containers (moved from apps/ui-admin in the composition-root migration). LOWERED from 595 after PlacementAuthoringRow was extracted; adding the three-axis plant vector had taken it to 613 and this gate refused to be widened" },
  ...
  "packages/openclinxr/xr-scene/src/isolated-subject-lab.ts": { maxLines: 581, reason: "isolated subject harness lab (moved from apps/ui-xr in the composition-root migration) — split boot/render/evidence next" },
