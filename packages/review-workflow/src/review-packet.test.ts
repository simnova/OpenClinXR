  ...
  it("reconstructs a review timeline with trace quality and patient note evidence", () => {
    const packet = buildReviewPacket({
      ...
      traceEvents: [
        ...
        { sequence: 5, eventType: "learner.order", source: "learner", tag: "ecg_request", atSecond: 500 },
        { sequence: 6, eventType: "unsafe.timeout", source: "system", atSecond: 961 },
        { sequence: 7, eventType: "note.submitted", source: "learner", tag: "patient_note_submitted", atSecond: 1260 },
      ],
      stationRunId: "run_001",
      patientNote: {
        stationRunId: "run_001",
    ...
    expect(packet.timeline.map((entry) => entry.sequence)).toEqual([0, 3, 4, 5, 6, 7]);
