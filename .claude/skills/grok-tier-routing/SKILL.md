---
name: grok-tier-routing
description: Grok-harness-only model tier ladder, spawn surfaces, and per-slice token introspection. Load before spawning a Grok subagent, choosing explore/plan/general-purpose, running pnpm grok:tier:* commands, or debugging headless -p behaviour (personas, capability_mode, Stop hooks, subagent model routing). Does NOT apply to Codex, Cursor, or Claude harnesses.
---
...
| Grok Rhai `workflow` tool | **WORKS headless** — deterministic `.grok/workflows/*.rhai` (agent/parallel/phase/complete + output_schema) run to completion in `-p`; tool present with no env gate (but `agent()` fan-out that spawns needs `GROK_SUBAGENTS=1`) | Named project workflows need folder trust (inline `script:` doesn't); tool is NON-BLOCKING (poll `state.json`). Use for bounded parallel scoring/proposals (top pick: vision score-and-rank). Proof: agentic-eval `docs/findings/grok-workflows.md` |
| `resume_from` / `grok -p --resume <sessionId>` | **WORKS headless** — continue a subagent thread with a short delta (~13–25% token save on multi-round) OR reconnect a NEW process to a prior session's subagents (survives process exit; VERIFIED recall across processes) | Session-scoped (new session w/o `--resume` → "not found"); persist `sessionId` + latest `subagent_id`. Use for iterative asset refinement / steering long efforts. Proof: agentic-eval `docs/findings/resume-chaining.md` |
