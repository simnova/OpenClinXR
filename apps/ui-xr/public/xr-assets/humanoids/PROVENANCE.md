...
- SHA-256: `e6a2c02330561b57dcd08837a8b4752f901fb9fa1320ef9c56feb75cf8cd2e1e`
...
- **2026-08-10 regeneration (#222):** the candidate was regenerated by the same generator so the MPFB rail paints the proven bounds-derived scalp/hair material region (imported from `tools/openclinxr/asset-pipeline/anny/automate_blender.py`) onto the body mesh instead of a hand-authored UV sphere. SHA-256 changed to `80a35ed7bcec84623032b0592881bf459c3b73d63e81df067d4a5e384e83fbbb`. No texture or rig changes.
- **2026-08-11 regeneration (#317):** the generator replaced the MPFB UI operator (`bpy.ops.mpfb.load_face_shape_keys`, which reads panel state and does nothing headless) with the proven `TargetService.load_target` path from `body_param_stage.load_mpfb_face_shape_keys`, and corrected the #222 scalp-region Z-flip (create_human faces -Y, not +Y — the flip pushed the face to +Y so the face-band exclusion never fired and the scalp covered the eyes, stranding their morph deltas). The body now ships 42 morph targets (10 macro-detail + 32 MPFB FACS: 13 mouth, 6 eye, 8 eyebrow, 4 nose, 1 neck); all 32 FACS targets are non-empty and localised. SHA-256 changed to `e6a2c02330561b57dcd08837a8b4752f901fb9fa1320ef9c56feb75cf8cd2e1e` (7,591,608 bytes). No texture or rig changes.
...
- Promoted byte-identical copy of the candidate to `generated-humanoids/mpfb-ob-patient-aisha.glb` (SHA-256 `3db127303027d0e89e155eb637e8efe6fb1cad84efaf4b36f5fdd0bfe9975635`, 5,406,792 bytes after the #318 helper-shell strip + #321 fitted toigo t-shirt + #321 placement fix; re-promoted byte-identical on 2026-08-11) and wired the OB triage patient (`patient_aisha_khan_v1` in `ob_headache_preeclampsia_triage_v1`) as the first MPFB cast.
- **2026-08-11 regeneration (#323):** the generator now wires the PROVEN body-part-hiding tool from the sibling rail — `body_param_stage.apply_body_hide_material_region` (body_param_stage.py:651) with the per-triangle mask from `garment_coverage.body_hide_mask` (signed clearance < 5 mm against the body's outward normal), scoped away from the hands with `scope_hide_mask_away_from_hands` (the #295 stump refused on this rail too). The MPFB2 rail had no body-part hiding, so the fitted t-shirt and the body it is fitted to both rendered and the body poked through the cloth across chest, abdomen, shoulders and collar; hiding is the §6s industry answer (hide the body under cloth) and #322's 1.5 cm standoff was already in the bytes. The glTF exporter maps the constant alpha-0 Principled input to `alphaMode=MASK / alphaCutoff=0.5` so hidden faces are DISCARDED. Measured: 1,605 hidden verts / 2,636 tris (10.5% of the body, vs the library rail's known-good 10.1%), 0 hidden hand verts, garment unchanged. SHA-256 changed to `2494318b8474b63fe00509fb1ef8faafa479bdeafb73ecf460999b64a45b69a1` (5,590,856 bytes). Geometry, rig, shape keys and garment placement unchanged.
- **2026-09-03 chest-anchor separation (#0):** `tools/openclinxr/asset-pipeline/anny/separate_chest_anchor_joints.mjs` mirrored breast.L / breast.R to x = +/- 0.085 m (was 0.00000 both — the MPFB addon rig data authors both breast heads at the spine AABB centre, i.e. the midline). The translation keyframes of both joints moved with the rest nodes; y/z, skin, meshes, materials, other joints and rotation/scale channels unchanged. SHA-256 changed to `1ac445b1412f6c1668923dd534c92f1871b8a16cbbd9bc0ce7e2cc89e583d260`. Report: `tools/openclinxr/evidence/chest-anchor-joints/mpfb-ob-patient-aisha-rigged-candidate.json`.
- **2026-08-11 placement fix (#321 handback):** the orchestrator graded the pixels — the first fit's garment was lying on the floor (Y/Z swapped: garment Y [-0.16, 0.08] at floor level). Cause: `import_obj(force_z=False)` leaves the OBJ importer's 90-degree X rotation on the garment OBJECT, while the `create_human()` body bakes that rotation into mesh data (`transform_apply` in MPFB's `load_wavefront_file`); `ClothesService.fit_clothes_to_human` writes body-local coords into the garment, so a rotated garment object renders them rotated. Fix: `apply_object_transforms(garment)` immediately after import (the same bake the body receives), before the fit. Post-fix garment Y [0.97, 1.42] overlaps the rig torso band (pelvis 0.874 -> clavicle 1.342 from the skin's inverse bind matrices). A placement clause (5) in `mpfb2-actor-wears-a-fitted-garment.test.ts` derives the band from the IBM and refuses a floor-level garment; it failed on the pre-fix bytes and passes now.
- **2026-08-11 regeneration (#321):** the generator fits a real MakeHuman garment on the helper-stripped basemesh via the PROVEN `ClothesService.fit_clothes_to_human` path (the same service `body_param_stage.py` uses for the hm08 library rail; D1 — not hand-authored geometry). The CC0 `toigo_basic_tucked_t-shirt` is fitted AFTER the #318 strip because `.mhclo` vertex refs index the canonical 13,380-vert hm08 topology (toigo max ref 11,017 < 13,380); the polo (3,648 refs into helper verts) is refused. Garment 2,700 tris / 5,400 exported verts, skinned to the standard rig via k-NN body-group projection; body stays 26,756 tris; 13 usable mouth targets and 137 joints unchanged. Garment choice is the least-wrong OB triage staging: a hospital gown is not in the cached library and a scrub shirt is staff wear.
