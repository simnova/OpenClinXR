...
const PAYLOAD_PHASE: Record<string, LearnerExamFlowPhase> = { "encounter.started": "encounter", "encounter.ended": "encounter", "note.started": "note", "note.submitted": "note", "station.advanced": "complete" };
export type LearnerCanonicalPhaseTraceStore = { examRunId: string; stationRunId: string; scenarioId: string; stationOrder: number; persistedEvents: TraceEvent[]; localEvents: TraceEvent[] };
...
export function createLearnerCanonicalPhaseTraceStore(input: Omit<LearnerCanonicalPhaseTraceStore, "persistedEvents" | "localEvents">): LearnerCanonicalPhaseTraceStore { return { ...input, persistedEvents: [], localEvents: [] }; }
export function createLearnerCanonicalPhaseEvent(input: { stationRunId: string; sequence: number; eventType: (typeof LEARNER_CANONICAL_PHASE_TYPES)[number]; atSecond: number; scenarioId: string; examRunId: string; stationOrder: number; phase: LearnerExamFlowPhase; formAtSecond: number; advanceReason?: string }): TraceEvent {
  ...
  return { stationRunId: input.stationRunId, sequence: input.sequence, eventType: input.eventType, occurredAt: new Date(atSecond * 1000).toISOString(), atSecond, source: "learner_ui", payload: { scenarioId: input.scenarioId, examRunId: input.examRunId, stationOrder: input.stationOrder, phase: input.phase, formAtSecond: Math.max(0, Math.floor(input.formAtSecond)), provenance: "local_exam_flow_fallback", ...(input.advanceReason ? { advanceReason: input.advanceReason } : {}) } };
}
export function admitLearnerCanonicalPhaseEvent(store: LearnerCanonicalPhaseTraceStore, event: TraceEvent): { ok: true; store: LearnerCanonicalPhaseTraceStore } | { ok: false; reason: "non_monotonic_sequence" | "cross_run" | "unknown_event_type" | "missing_identity" | "malformed_durable_ref"; store: LearnerCanonicalPhaseTraceStore } {
  ...
  const examRunId = String(event.payload["examRunId"] ?? ""); const scenarioId = String(event.payload["scenarioId"] ?? ""); const stationOrder = Number(event.payload["stationOrder"] ?? Number.NaN); const formAtSecond = Number(event.payload["formAtSecond"]); const occurredAtMs = typeof event.occurredAt === "string" ? Date.parse(event.occurredAt) : Number.NaN;
  if (!examRunId || !scenarioId || !event.stationRunId || !Number.isInteger(stationOrder) || !Number.isInteger(event.sequence) || event.sequence < 0 || !Number.isInteger(event.atSecond) || event.atSecond < 0 || !Number.isInteger(formAtSecond) || formAtSecond < 0 || !Number.isFinite(occurredAtMs)) return { ok: false, reason: "missing_identity", store };
  if (examRunId !== store.examRunId || event.stationRunId !== store.stationRunId || scenarioId !== store.scenarioId || stationOrder !== store.stationOrder) return { ok: false, reason: "cross_run", store };
  if (event.payload["durableEventRef"] !== `durable://station-runs/${store.stationRunId}/events/${event.sequence}` || event.payload["phase"] !== PAYLOAD_PHASE[event.eventType]) return { ok: false, reason: "malformed_durable_ref", store };
  const last = store.persistedEvents[store.persistedEvents.length - 1];
  if (last ? (event.sequence <= last.sequence || event.atSecond < last.atSecond || formAtSecond < Number(last.payload["formAtSecond"]) || occurredAtMs < Date.parse(last.occurredAt) || (PHASE_RANK[event.eventType] ?? -1) !== (PHASE_RANK[last.eventType] ?? -1) + 1) : (PHASE_RANK[event.eventType] ?? -1) > 1) return { ok: false, reason: "non_monotonic_sequence", store };
  return { ok: true, store: { ...store, persistedEvents: [...store.persistedEvents, event] } };
}
export function restoreLearnerCanonicalPhaseTraceFromJson(store: LearnerCanonicalPhaseTraceStore, raw: string | null): LearnerCanonicalPhaseTraceStore {
  try { if (!raw) return store; const parsed = JSON.parse(raw) as unknown; const record = parsed && typeof parsed === "object" && !Array.isArray(parsed) ? parsed as { persistedEvents?: TraceEvent[]; localEvents?: TraceEvent[] } : null; const local = Array.isArray(parsed) ? parsed as TraceEvent[] : (Array.isArray(record?.localEvents) ? record.localEvents : []); let current: LearnerCanonicalPhaseTraceStore = { ...store, persistedEvents: [], localEvents: [] }; for (const item of record?.persistedEvents ?? []) { const admitted = admitLearnerCanonicalPhaseEvent(current, item); if (admitted.ok) current = admitted.store; } return { ...current, localEvents: local.filter((item) => item && typeof item === "object") }; } catch { return store; }
}
export function viewLearnerCanonicalExamPhase(store: LearnerCanonicalPhaseTraceStore): LearnerCanonicalExamPhaseView {
  const fallbackActive = store.persistedEvents.length === 0; const events = fallbackActive ? store.localEvents : store.persistedEvents; const last = events[events.length - 1];
  const ofType = (eventType: string) => [...events].reverse().find((event) => event.eventType === eventType); const advanced = ofType("station.advanced");
  const reason = typeof advanced?.payload["advanceReason"] === "string" ? String(advanced.payload["advanceReason"]) : typeof last?.payload["advanceReason"] === "string" ? String(last.payload["advanceReason"]) : last ? last.eventType : null;
  return { source: fallbackActive ? "local_exam_flow_fallback" : "canonical_assembled_exam_phase_trace", fallbackActive, fallbackLabel: fallbackActive ? "local-only fallback — no admitted canonical phase trace for this exam run" : null, examRunId: store.examRunId, scenarioId: store.scenarioId, stationOrder: store.stationOrder, phase: last ? (PHASE_FROM_TYPE[last.eventType] ?? "encounter") : "encounter", lastAdvanceReason: reason, lastAdmittedSequence: last?.sequence ?? null, admittedCount: events.length, encounterEndedAtSecond: ofType("encounter.ended")?.atSecond ?? null, noteStartedAtSecond: ofType("note.started")?.atSecond ?? null, noteSubmitted: events.some((event) => event.eventType === "note.submitted"), examEquivalenceGate: false, claimBoundary: "learner_ui_xr_canonical_phase_trace_not_exam_equivalence", notEvidenceFor: LEARNER_EXAM_PHASE_NOT_EVIDENCE_FOR };
}
...
function withLocal(store: LearnerCanonicalPhaseTraceStore, event: TraceEvent): LearnerCanonicalPhaseTraceStore { return { ...store, localEvents: [...store.localEvents, event] }; }
export function applyLearnerExamFlowIntent(store: LearnerCanonicalPhaseTraceStore, intent: LearnerExamFlowIntent): LearnerCanonicalPhaseApplyResult {
  const view = viewLearnerCanonicalExamPhase(store); const seq = (store.localEvents[store.localEvents.length - 1]?.sequence ?? -1) + 1;
  ...
  if (intent.kind === "end_encounter" || intent.kind === "encounter_timer_elapsed") {
    ...
    const reason = intent.kind === "encounter_timer_elapsed" ? "encounter_timer_elapsed_note_phase_started" : "encounter_ended_note_phase_started";
    const next = withLocal(withLocal(store, createLearnerCanonicalPhaseEvent({ ...base, sequence: seq, eventType: "encounter.ended", phase: "encounter", advanceReason: reason })), createLearnerCanonicalPhaseEvent({ ...base, sequence: seq + 1, eventType: "note.started", phase: "note", advanceReason: reason }));
    return { store: next, view: viewLearnerCanonicalExamPhase(next), admitted: true, refusalReason: null, navigateToScenarioId: null };
  }
  ...
  const advanceReason = intent.kind === "note_timer_elapsed" ? (intent.nextScenarioId ? "note_timer_elapsed_advancing" : "note_timer_elapsed_last_station_complete") : (intent.nextScenarioId ? "patient_note_submitted_advancing" : "last_station_note_submitted_exam_complete");
  const next = withLocal(withLocal(store, createLearnerCanonicalPhaseEvent({ ...base, sequence: seq, eventType: "note.submitted", phase: "note", advanceReason })), createLearnerCanonicalPhaseEvent({ ...base, sequence: seq + 1, eventType: "station.advanced", phase: "complete", advanceReason }));
  return { store: next, view: viewLearnerCanonicalExamPhase(next), admitted: true, refusalReason: null, navigateToScenarioId: intent.nextScenarioId };
}
