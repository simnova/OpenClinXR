...
export function admitLearnerCanonicalPhaseEvent(store: LearnerCanonicalPhaseTraceStore, event: TraceEvent): { ok: true; store: LearnerCanonicalPhaseTraceStore } | { ok: false; reason: "non_monotonic_sequence" | "cross_run" | "unknown_event_type" | "missing_identity" | "malformed_durable_ref"; store: LearnerCanonicalPhaseTraceStore } {
  ...
  const examRunId = String(event.payload["examRunId"] ?? ""); const scenarioId = String(event.payload["scenarioId"] ?? ""); const stationOrder = Number(event.payload["stationOrder"] ?? Number.NaN);
  if (!examRunId || !scenarioId || !event.stationRunId || !Number.isFinite(stationOrder) || typeof event.occurredAt !== "string" || event.occurredAt.length === 0) return { ok: false, reason: "missing_identity", store };
