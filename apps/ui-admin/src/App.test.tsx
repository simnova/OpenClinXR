  ...
  return {
    ...
    submitScenarioSceneGenerationMaterializationInputReview: async (input) => ({
      ...
      decisions: input.decisions.map((decision) => ({
        ...
        reviewerId: "admin_materialization_input_reviewer",
    ...
    submitRuntimeRealismEvidenceInputReview: async (input) => ({
      schemaVersion: "openclinxr.runtime-realism-evidence-input-review-decision-record.v1",
      source: "admin_runtime_realism_evidence_input_review_decisions",
      scenarioId: input.scenarioId ?? "peds_asthma_parent_anxiety_v1",
      decisionCount: input.decisions.length,
      reviewedDecisionCount: input.decisions.filter((decision) => decision.decision === "reviewed_metadata_only").length,
      heldDecisionCount: input.decisions.filter((decision) => decision.decision === "held_metadata_only").length,
      decisions: input.decisions.map((decision) => ({
        ...
        reviewerId: "admin_runtime_realism_evidence_reviewer",
        decision: decision.decision,
        comments: decision.comments,
        evidenceRefs: decision.evidenceRefs,
        reviewedAt: "2026-05-28T10:15:00.000Z",
      })),
      providerExecutionAllowed: false,
      runtimeExecutionAllowed: false,
      learnerLaunchAllowed: false,
    ...
    getReviewReplayReadinessSummary: async (input) => ({
      ...
      runtimeVisualEvidenceReplayProjection: {
        ...
        blockerIds: ["runtime_realism_evidence_not_attached_to_encounter_bundle"],
        nextActions: [
          "review 3 accepted metadata-only runtime/visual refs before scenario iteration",
        ...
        uiXrConsumerOperatorWorkflowSummary: {
          ...
          submitPreview: {
            ...
            inputIds: ["runtime-realism-evidence-input:patient_ed_chest_pain_v1"],
            localArtifactPaths: ["metadata-only-admin-review/runtime-realism-evidence-input_patient_ed_chest_pain_v1.json"],
            rawPayloadDisplayed: false,
          ...
          preflightChecks: ["scenario_id_matches_payload_and_expected_scenario", "raw_payload_hidden"],
          nextActions: ["submit metadata-only UI-XR runtime/visual refs through the guarded attachment route"],
          rawPayloadDisplayed: false,
          providerExecutionAllowed: false,
          runtimeExecutionAllowed: false,
          learnerLaunchAllowed: false,
          ...
          notEvidenceFor: ["provider_availability", "runtime_readiness", "learner_launch_readiness", "quest_readiness", "production_asset_readiness", "clinical_validity", "scoring_validity"],
        },
    ...
    persistFacultyScoreDraft: async (input) => ({
      stationRunId: input.stationRunId,
      scenarioId: "ed_chest_pain_priority_v1",
      draftId: `faculty_score_draft:${input.stationRunId}:test`,
    ...
    saveFacultyReviewDecision: async (input) => ({
      stationRunId: input.stationRunId,
      scenarioId: "ed_chest_pain_priority_v1",
      decisionId: `faculty_review_decision:${input.stationRunId}:test`,
      savedAt: "2026-08-04T00:00:00.000Z",
      localDecision: input.localDecision ?? "hold",
      facultyScoreDraft: {
        reviewerId: input.reviewerId,
        status: "draft" as const,
        comments: input.comments ?? "",
      ...
      runtimePromotionAllowed: false as const,
      productionManifestPromotionAllowed: false as const,
      scoringValidityClaimed: false as const,
